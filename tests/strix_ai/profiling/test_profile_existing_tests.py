...
class TestProfileViT:
    ...
    def test_profile_vit_inference(self, cleanup_gpu):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "vit_traces"
            output_dir.mkdir()
...
class TestProfileCV:
    ...
    def test_profile_yolo(self, cleanup_gpu):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "yolo_traces"
            output_dir.mkdir()
...
class TestProfileVLA:
    ...
    def test_profile_vla(self, cleanup_gpu):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "vla_traces"
            output_dir.mkdir()
...
class TestProfileAll:
    ...
    def test_profile_all_categories(self, cleanup_gpu):
        ...
        categories = {
            "vlm": "tests/strix_ai/vlm/",
            "vit": "tests/strix_ai/vit/",
            "cv": "tests/strix_ai/cv/",
            "vla": "tests/strix_ai/vla/",
        }
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for category, test_path in categories.items():
                ...
                output_dir = Path(tmpdir) / f"{category}_traces"
                output_dir.mkdir()
                ...
                print(f"✅ {category}: {len(trace_files)} trace files generated")
...
class TestProfileQuick:
    ...
    def test_profile_quick_smoke(self, cleanup_gpu):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "quick_traces"
            output_dir.mkdir()
            
            # Profile a quick existing test (use a marker to run quick tests)
            test_path = "tests/strix_ai/vlm/ -m quick"
            
            print(f"📊 Profiling quick tests: {test_path}")
