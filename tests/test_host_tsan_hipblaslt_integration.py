...
class HostTsanHipblasltIntegrationTest(unittest.TestCase):
    def test_host_tsan_enables_upstream_codegen_launcher(self):
        source = (
            THEROCK_DIR / "math-libs" / "BLAS" / "pre_hook_hipBLASLt.cmake"
        ).read_text(encoding="utf-8")

        host_tsan_block = re.search(
            r'if\(THEROCK_SANITIZER STREQUAL "HOST_TSAN"\)(.*?)endif\(\)',
            source,
            re.DOTALL,
        )
        self.assertIsNotNone(host_tsan_block)
        self.assertRegex(
            host_tsan_block.group(1),
            r"set\(HIPBLASLT_ENABLE_TSAN ON CACHE BOOL\s+"
            r'"[^"]+" FORCE\)',
        )
