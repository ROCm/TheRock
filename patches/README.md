...
### Skipping patches temporarily with SKIP_PATCHES

When removing patches for external repositories (rocm-libraries, rocm-systems), there is a chicken-and-egg problem:

- TheRock can't remove the patch until the external repo has the fix
- External repo CI can't pass until TheRock removes the patch

**Solution**: Use the `SKIP_PATCHES` environment variable or `skip_patches` workflow input to temporarily skip specific patches during coordinated removal.

**For workflow_call users** (external repos calling TheRock's CI):
...
**Testing in a PR** (before merging workflow changes):

External repos can test TheRock changes by temporarily modifying their workflow to point to a TheRock branch:

```yml
# In external repo PR, test against TheRock branch
jobs:
  build:
    uses: ROCm/TheRock/.github/workflows/ci.yml@users/someone/some-feature
    with:
      external_source_checkout: true
      repository_override: ${{ github.repository }}
      skip_patches: "0001-workaround.patch"
```

Once tested, revert the TheRock ref to main/stable before merging.
...
1. **External repo PR**: Merge the fix + add `skip_patches` to workflow, verify CI passes
1. **TheRock PR**: Update submodule to pick up the fix from Step 1 + delete the patch file(s), verify external repo CI still passes
1. **External repo cleanup PR**: Remove `skip_patches` from workflow (no longer needed)

This three-step process ensures both repos remain buildable throughout the patch removal.
