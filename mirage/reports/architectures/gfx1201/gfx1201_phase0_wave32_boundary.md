...
- Phase-0 executable opcodes: `333`
- Wave size: `32`
...
- `ENC_SMEM` now has eight local executable footholds via `S_DCACHE_INV`, `S_PREFETCH_INST`, `S_PREFETCH_INST_PC_REL`, `S_PREFETCH_DATA`, `S_BUFFER_PREFETCH_DATA`, `S_PREFETCH_DATA_PC_REL`, `S_ATC_PROBE`, and `S_ATC_PROBE_BUFFER`, but the remaining `20` seeded scalar-memory instructions stay scaffolded.
...
- `ENC_SMEM`: example `S_LOAD_B32`, seeded `28`, executable `8` via `S_ATC_PROBE`, as-is `0`, decoder-rollup `3`, semantic-only `0`, gfx1201-specific `25`
...
smallest remaining seeded blocker and the current local cache-hint/probe footholds keep the
