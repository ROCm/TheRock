...
- Phase-0 executable opcodes: `339`
- Wave size: `32`
...
- `ENC_SMEM` now has fourteen local executable footholds via `S_DCACHE_INV`, `S_PREFETCH_INST`, `S_PREFETCH_INST_PC_REL`, `S_PREFETCH_DATA`, `S_BUFFER_PREFETCH_DATA`, `S_PREFETCH_DATA_PC_REL`, `S_ATC_PROBE`, `S_ATC_PROBE_BUFFER`, `S_LOAD_B32`, `S_LOAD_B64`, `S_LOAD_I8`, `S_LOAD_U8`, `S_LOAD_I16`, and `S_LOAD_U16`, but the remaining `14` seeded scalar-memory instructions stay scaffolded.
...
- `ENC_SMEM`: example `S_LOAD_B32`, seeded `28`, executable `14` via `S_ATC_PROBE`, as-is `0`, decoder-rollup `3`, semantic-only `0`, gfx1201-specific `25`
...
smallest remaining seeded blocker and the current local cache-hint/probe plus first-load footholds keep the
