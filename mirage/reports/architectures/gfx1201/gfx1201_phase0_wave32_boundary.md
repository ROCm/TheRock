...
- Phase-0 executable opcodes: `384`
- Wave size: `32`
...
- `ENC_VGLOBAL`: example `GLOBAL_LOAD_B32`, seeded `65`, executable `31` via `GLOBAL_INV`, as-is `3`, decoder-rollup `0`, semantic-only `0`, gfx1201-specific `62`
...
the plain `GLOBAL_STORE_B8`/`B16`/`B32`/`B64`/`B96`/`B128` slice, `GLOBAL_STORE_ADDTID_B32`, `GLOBAL_STORE_BLOCK`, and `GLOBAL_STORE_D16_HI_B8`/`B16`,
leaving the atomic vector-memory surface as the remaining local `ENC_VGLOBAL` boundary.
