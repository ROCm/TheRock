...
- Phase-0 executable opcodes: `382`
- Wave size: `32`
...
- `ENC_VGLOBAL`: example `GLOBAL_LOAD_B32`, seeded `65`, executable `29` via `GLOBAL_INV`, as-is `3`, decoder-rollup `0`, semantic-only `0`, gfx1201-specific `62`
...
`GLOBAL_LOAD_TR_B64`/`B128`, the packed `GLOBAL_LOAD_D16_U8`/`I8`/`B16` and `GLOBAL_LOAD_D16_HI_U8`/`I8`/`B16` slice,
the plain `GLOBAL_STORE_B8`/`B16`/`B32`/`B64`/`B96`/`B128` slice, `GLOBAL_STORE_ADDTID_B32`, and `GLOBAL_STORE_D16_HI_B8`/`B16`
without committing to broader vector-memory `BLOCK` or atomic semantics.
