...
- Phase-0 executable opcodes: `370`
- Wave size: `32`
...
- `ENC_VGLOBAL`: example `GLOBAL_LOAD_B32`, seeded `65`, executable `17` via `GLOBAL_INV`, as-is `3`, decoder-rollup `0`, semantic-only `0`, gfx1201-specific `62`
...
the plain `GLOBAL_LOAD_U8`/`I8`/`U16`/`I16`/`B32`/`B64`/`B96`/`B128` slice, and the matching
`GLOBAL_STORE_B8`/`B16`/`B32`/`B64`/`B96`/`B128` slice without committing to broader vector-memory
atomics, `D16`, or `BLOCK` semantics. The later frontier steps move into broader
