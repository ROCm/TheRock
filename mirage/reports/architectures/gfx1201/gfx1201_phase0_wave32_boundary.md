...
- Phase-0 executable opcodes: `415`
- Wave size: `32`
...
- `ENC_VGLOBAL`: example `GLOBAL_LOAD_B32`, seeded `65`, executable `62` via `GLOBAL_ATOMIC_ADD_F32`, as-is `3`, decoder-rollup `0`, semantic-only `0`, gfx1201-specific `62`
...
and the `F32` atomic slice `GLOBAL_ATOMIC_ADD_F32` and `GLOBAL_ATOMIC_MIN/MAX_NUM_F32`,
leaving the packed-half and ordered-add atomic vector-memory surface as the remaining local `ENC_VGLOBAL` boundary.
