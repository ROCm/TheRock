...
252 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- The next coherent extension point is the remaining adjacent `ENC_VOP1` consumer or half-width seed work after the local `F16` compare/class batch, or the remaining packed/wider `ENC_VOPC` compare work, before `ENC_VOP3`/half-conversion work.
