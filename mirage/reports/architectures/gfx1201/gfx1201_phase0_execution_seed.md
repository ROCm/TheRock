...
370 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- `GLOBAL_LOAD_U8`, `GLOBAL_LOAD_I8`, `GLOBAL_LOAD_U16`, `GLOBAL_LOAD_I16`, `GLOBAL_LOAD_B32`, `GLOBAL_LOAD_B64`, `GLOBAL_LOAD_B96`, `GLOBAL_LOAD_B128`, `GLOBAL_STORE_B8`, `GLOBAL_STORE_B16`, `GLOBAL_STORE_B32`, `GLOBAL_STORE_B64`, `GLOBAL_STORE_B96`, and `GLOBAL_STORE_B128` now decode and execute as the first real wave32-local per-lane global-memory reads and writes on both the decoded and compiled seed paths, combining `VADDR`, scalar `SADDR`, and inline offsets through `ExecutionMemory` without taking on the broader atomic or packed-memory surface.
...
- `ENC_VGLOBAL` now has executable footholds through `GLOBAL_INV`, `GLOBAL_WB`, `GLOBAL_WBINV`, the plain `GLOBAL_LOAD_U8`/`I8`/`U16`/`I16`/`B32`/`B64`/`B96`/`B128` slice, and the matching `GLOBAL_STORE_B8`/`B16`/`B32`/`B64`/`B96`/`B128` slice, but the broader `D16`, `BLOCK`, `ADDTID`, and atomic global-memory seed surface is still route-only in this slice.
