...
- `S_CMP_EQ_U32` and `S_CMP_LG_U32` decode from `ENC_SOPC`.
- `S_BRANCH`, `S_CBRANCH_SCC0`, and `S_CBRANCH_SCC1` decode from `ENC_SOPP` with sign-extended relative deltas.
...
- Scalar compare updates `SCC` for branch plumbing.
- `S_BRANCH`, `S_CBRANCH_SCC0`, and `S_CBRANCH_SCC1` update `pc` on the compiled seed path using relative instruction deltas.
