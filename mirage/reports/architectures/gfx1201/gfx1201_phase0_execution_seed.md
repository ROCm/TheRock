...
378 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- `ENC_VGLOBAL` now has executable footholds through `GLOBAL_INV`, `GLOBAL_WB`, `GLOBAL_WBINV`, the plain `GLOBAL_LOAD_U8`/`I8`/`U16`/`I16`/`B32`/`B64`/`B96`/`B128` slice, the packed `GLOBAL_LOAD_D16_U8`/`I8`/`B16` and `GLOBAL_LOAD_D16_HI_U8`/`I8`/`B16` slice, the plain `GLOBAL_STORE_B8`/`B16`/`B32`/`B64`/`B96`/`B128` slice, and `GLOBAL_STORE_D16_HI_B8`/`B16`, but the broader `TR`, `BLOCK`, `ADDTID`, and atomic global-memory seed surface is still route-only in this slice.
