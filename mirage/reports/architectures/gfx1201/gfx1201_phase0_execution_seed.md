...
326 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- `ENC_SMEM` is now partially bootstrapped through `S_DCACHE_INV`, but the remaining scalar-memory seed surface stays scaffolded.
...
- The next coherent extension points now deepen `ENC_SMEM` beyond `S_DCACHE_INV`, or move into `ENC_VOP3`, `ENC_VDS`, and `ENC_VGLOBAL`, rather than more narrow `VOP1`/`VOP2`/`VOPC` seed churn.
