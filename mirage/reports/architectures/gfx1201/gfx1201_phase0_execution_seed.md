...
475 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- The next coherent extension points now move further into `ENC_VDS` through nearby single-address utilities or the remaining low-risk packed LDS tail, with `ENC_VOP3` remaining the later broader frontier.
