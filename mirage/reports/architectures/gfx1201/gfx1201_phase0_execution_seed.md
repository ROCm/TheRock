...
339 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- `ENC_SOP1`: `S_MOV_B32`
- `ENC_SOPK`: `S_MOVK_I32`
- `ENC_SMEM`: `S_DCACHE_INV`, `S_PREFETCH_INST`, `S_PREFETCH_INST_PC_REL`, `S_PREFETCH_DATA`, `S_BUFFER_PREFETCH_DATA`, `S_PREFETCH_DATA_PC_REL`, `S_ATC_PROBE`, `S_ATC_PROBE_BUFFER`, `S_LOAD_B32`, `S_LOAD_B64`, `S_LOAD_I8`, `S_LOAD_U8`, `S_LOAD_I16`, `S_LOAD_U16`
...
- `S_LOAD_B32`, `S_LOAD_B64`, `S_LOAD_I8`, `S_LOAD_U8`, `S_LOAD_I16`, and `S_LOAD_U16` now decode from `ENC_SMEM` as the first real scalar-memory dataflow seeds, keeping 64-bit scalar bases, inline or scalar-register offsets, narrow sign/zero extension, and 32/64-bit scalar destinations local to the phase-0 path.
...
- `S_LOAD_B32`, `S_LOAD_B64`, `S_LOAD_I8`, `S_LOAD_U8`, `S_LOAD_I16`, and `S_LOAD_U16` now execute as the first real wave32-local scalar-memory reads on both the decoded and compiled seed paths, using `ExecutionMemory` plus local sign/zero extension without widening shared scalar-memory state.
...
- `ENC_SMEM` is now partially bootstrapped through `S_DCACHE_INV`, `S_PREFETCH_INST`, `S_PREFETCH_INST_PC_REL`, `S_PREFETCH_DATA`, `S_BUFFER_PREFETCH_DATA`, `S_PREFETCH_DATA_PC_REL`, `S_ATC_PROBE`, `S_ATC_PROBE_BUFFER`, `S_LOAD_B32`, `S_LOAD_B64`, `S_LOAD_I8`, `S_LOAD_U8`, `S_LOAD_I16`, and `S_LOAD_U16`, but the remaining `14` scalar-memory seed instructions stay scaffolded.
...
- The next coherent extension points now deepen `ENC_SMEM` beyond the current cache-hint/probe plus first-load footholds, or move into `ENC_VOP3`, `ENC_VDS`, and `ENC_VGLOBAL`, rather than more narrow `VOP1`/`VOP2`/`VOPC` seed churn.
