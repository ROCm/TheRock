...
353 executable phase-0 ops without leaving architecture-local `gfx1201` files, and
...
- The next coherent extension points now move past saturated `ENC_SMEM` into `ENC_VGLOBAL`, `ENC_VDS`, and `ENC_VOP3`, rather than more narrow `VOP1`/`VOP2`/`VOPC` seed churn.
