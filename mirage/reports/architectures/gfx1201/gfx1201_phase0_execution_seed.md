...
173 executable phase-0 ops without leaving architecture-local `gfx1201` files.
...
- `V_CVT_F32_F64`, `V_CVT_I32_F64`, and `V_CVT_U32_F64` now decode from `ENC_VOP1` with architecture-local wide source descriptors for SGPR/VGPR register pairs.
...
- `V_CVT_F32_F64`, `V_CVT_I32_F64`, and `V_CVT_U32_F64` execute lane-wise under `exec_mask` with gfx950-consistent double-to-float and double-to-int truncation behavior.
