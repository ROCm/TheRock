...
- Route manifests:
  ...
  - exact route-info negative-path checks now pin supported synthetic `StubDecoderRouteInfo` inputs with invalid caller decode hints, empty names, synthetic unknown names, a clean unsupported-seeded representative (`V_CVT_SCALEF32_PK8_FP8_F32`), and lowercased / leading-whitespace near-miss known opcodes to preserve caller route metadata while returning either routed structural parity or an empty `kUnknown` structural decode through the route-specific entrypoint name
