...
- Route manifests:
  ...
  - exact route-info negative-path checks now pin supported synthetic `StubDecoderRouteInfo` inputs with both synthetic unknown names and lowercased / leading-whitespace near-miss known opcodes to preserve caller route metadata while returning an empty `kUnknown` structural decode through the route-specific entrypoint name
