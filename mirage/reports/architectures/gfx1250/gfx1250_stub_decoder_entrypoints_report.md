...
- Stub decode results now include:
  ...
  - route-wide exact operand-role binding count and output-flag validation across the current routed matrix and non-matrix seed sets
  ...
  - exact scale-paired tail-batch validation now isolates the routed 50-seed tail after the paired-load anchors: the routed `V_CVT_SCALE*` / `V_CVT_SR*` exclusions stay exact, and `V_DIV_SCALE_F64` plus the routed `V_WMMA_SCALE*` forms keep exact route-keyed parity, selector/manifest consistency, route-manifest count parity, and local operand-layout / descriptor exactness
...
- Route manifests:
  ...
  - exact leading `SeedFamily::kVop3p` 50-slice route-surface validation now pins selector, route-info, and manifest parity across the routed packed BF16, SWMMAC, and early WMMA forms
  ...
  - exact leading `SeedFamily::kFp8Bf8` 50-slice route-manifest count parity checks now pin the routed front half of the FP8/BF8 family against per-route instruction totals, selector consistency, and manifest accounting after the scale-paired tail layer
  ...
  - exact selector-to-decode parity checks now pin `SelectStubDecoderRoute(name)` / `FindStubDecoderRouteInfo(name)` against `DecodeStubInstruction(name)` status for routed seeds, unsupported seeded ops, unknown names, empty instruction names, lowercased known opcodes, whitespace-padded known opcodes, and prefixed, suffixed, or prefixed-plus-suffixed known-opcode near misses, including lowercase and whitespace-decorated variants of those near misses
  - exact selector negative-path checks now pin `SelectStubDecoderRoute(invalid_hint)`, `GetStubDecoderRouteName(invalid_route)`, `GetStubDecoderRouteInstructions(unsupported_or_invalid_route)`, `FindStubDecoderRouteManifest(unsupported_or_invalid_route)`, plus unknown-instruction, empty-instruction, lowercased-known-opcode, whitespace-padded-known-opcode, and prefixed, suffixed, or prefixed-plus-suffixed known-opcode lookup exclusion, including lowercase and whitespace-decorated near-miss variants
  - exact negative-path checks now pin `DecodeVop3pStub` / `DecodeMimgTensorStub` / `DecodeVop1Stub` / `DecodeVop3SdstStub` wrong-entrypoint rejection across the full routed seed set, pin both `DecodeStubInstruction()` and all four route-keyed entrypoints on the unsupported seeded slice, pin `DecodeStubInstruction(const StubDecoderRouteInfo&)` normalization for both unsupported and invalid route infos across valid and invalid caller decode hints, pin unknown-op, empty-instruction, lowercased-known-opcode, whitespace-padded-known-opcode, and prefixed, suffixed, or prefixed-plus-suffixed known-opcode route-keyed parity, including lowercase and whitespace-decorated near-miss variants, and pin null manifest lookups for both `kUnsupported` and invalid route enums
  ...
  - exact route-info negative-path checks now pin supported synthetic `StubDecoderRouteInfo` inputs with both invalid and valid-mismatching caller decode hints across routed, empty-name, synthetic-unknown, clean unsupported-seeded (`V_CVT_SCALEF32_PK8_FP8_F32`), and lowercased / leading-whitespace / prefixed / non-family-matched suffixed / prefixed-plus-suffixed near-miss known-opcode cases, including lowercase and whitespace-decorated near-miss variants, all while preserving caller route metadata and returning either routed structural parity or an empty `kUnknown` structural decode through the route-specific entrypoint name
  ...
  - exact scale-paired 50-slice batch validation now covers the post-pair-load `SeedFamily::kScalePaired` slice: the routed paired-load helpers `V_WMMA_LD_SCALE_PAIRED_B32` and `V_WMMA_LD_SCALE16_PAIRED_B64` stay architecture-local and exact, the deferred `V_CVT_SCALE*` and `V_CVT_SR*` forms stay unsupported and route-excluded, and `V_DIV_SCALE_F64` plus the routed `V_WMMA_SCALE*` forms keep exact route-keyed parity, selector/manifest consistency, local selector exclusion, and local operand-layout / descriptor exactness
  - exact scale-paired tail-batch validation now isolates the routed 50-seed tail after the paired-load anchors: the routed `V_CVT_SCALE*` / `V_CVT_SR*` exclusions stay exact, and `V_DIV_SCALE_F64` plus the routed `V_WMMA_SCALE*` forms keep exact route-keyed parity, selector/manifest consistency, route-manifest count parity, and local operand-layout / descriptor exactness
  - exact fp8/bf8 leading 50-slice route-manifest count parity validation now extends the routed `SeedFamily::kFp8Bf8` front half after the scale-paired tail layer: the routed `V_CVT_F16_*` / `V_CVT_PK_*` / `V_CVT_SCALE_PK*` front-half forms keep exact route-manifest count parity, selector consistency, and local route-info parity
  - exact fp8/bf8 leading 50-slice manifest-accounting validation now extends the routed `SeedFamily::kFp8Bf8` front half after the scale-paired tail layer: the routed front-half forms keep exact route-manifest provenance/count accounting, selector consistency, and local route-info parity
  - exact fp8/bf8 remaining-tail route-manifest count parity validation now extends the routed `SeedFamily::kFp8Bf8` tail after the leading-half layer: the routed `V_SWMMAC_*` / `V_WMMA_*` forms keep exact route-manifest count parity, selector consistency, and local route-info parity, while the deferred `V_CVT_SR_*` leftovers stay unsupported and route-excluded
  - exact fp8/bf8 remaining-tail manifest-accounting validation now layers route-info lookup parity and per-route provenance/count accounting onto the routed `SeedFamily::kFp8Bf8` tail after the route-manifest count parity layer, while the deferred `V_CVT_SR_*` leftovers stay unsupported and route-excluded
  - exact fp8/bf8 remaining-tail route-info fallback and near-miss validation now pins the deferred `V_CVT_SR_*` leftovers with selector exclusion, route-keyed unknown parity, and caller-metadata-preserving synthetic route-info unknowns after the manifest-accounting layer
  - exact fp8/bf8 first-50 batch validation now covers the leading `SeedFamily::kFp8Bf8` slice: the routed `V_CVT_F16_*` / `V_CVT_PK_*` forms keep exact route-keyed parity, the deferred `V_CVT_SCALE*` / `V_CVT_SR*` forms stay unsupported and route-excluded, and selector/manifest consistency now includes an explicit leading-batch route-surface check alongside local helper and operand exactness
  - exact fp8/bf8 trailing-slice route-surface validation now covers the remaining `SeedFamily::kFp8Bf8` tail: the routed `V_SWMMAC_*` / `V_WMMA_*` forms keep exact route-keyed parity and local operand exactness, while the deferred `V_CVT_SR_*` leftovers stay unsupported and route-excluded with preserved selector/manifest negative-path behavior
  - exact VOP3P first-50 batch validation now covers the leading `SeedFamily::kVop3p` slice through `V_WMMA_F32_16X16X4_F32_w32`: the routed packed BF16, SWMMAC, and early WMMA forms keep exact route-keyed parity, selector/manifest consistency, route-info payload parity, and local operand-layout / descriptor exactness
  ...
  - exact VOP3P tail-batch route-manifest count parity validation now pins the remaining routed `SeedFamily::kVop3p` tail against per-route instruction totals across the routed family set
...
- `VOP3P` tail-batch route-manifest count parity:
  - route-wide exact route-manifest count parity checks across the remaining `SeedFamily::kVop3p` tail batch
...
- Generic WMMA core variants:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across current routed non-scale `V_WMMA_*` seeds
...
- Generic WMMA scale variants:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across current routed `V_WMMA_SCALE*` seeds
...
- WMMA scale paired loads:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across current paired scale-load helpers
...
- Generic SWMMAC variants:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across current routed `V_SWMMAC_*` seeds
...
- Tensor routes:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across the current tensor route set
...
- VOP1 FP8/BF8 conversions:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across the current VOP1 route set
...
- VOP3 SDST scale assist:
  ...
  - Descriptor depth:
    ...
    - route-wide exact operand-role binding count and output-flag checks across the current VOP3 SDST route set
