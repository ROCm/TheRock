...
- Route manifests:
  ...
  - exact WMMA family route-surface validation now covers the routed `SeedFamily::kWmma` follow-on slice: the routed `V_WMMA_*` / `V_SWMMAC_*` forms and the tensor-memory anchors keep exact route-keyed parity, selector/manifest consistency, and local operand-layout / descriptor exactness
