...
- Route manifests:
  ...
  - exact routed-seed-catalog parity checks now pin route manifests, per-seed route-info fields, routed instruction-list membership, unsupported-seed exclusion, and route-manifest internal accounting (`instruction_count == xml_backed_count + llvm_only_count`, `target_specific_count <= instruction_count`) directly against `GetDecoderSeedInfos()` across `kVop3p`, `kMimgTensor`, `kVop1`, and `kVop3Sdst`
