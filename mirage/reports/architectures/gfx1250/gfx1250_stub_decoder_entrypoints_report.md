...
- Route manifests:
  ...
  - exact selector-to-decode parity checks now pin `SelectStubDecoderRoute(name)` / `FindStubDecoderRouteInfo(name)` against `DecodeStubInstruction(name)` status for routed seeds, unsupported seeded ops, unknown names, empty instruction names, lowercased known opcodes, whitespace-padded known opcodes, and prefixed or suffixed known-opcode near misses
  - exact selector negative-path checks now pin `SelectStubDecoderRoute(invalid_hint)`, `GetStubDecoderRouteName(invalid_route)`, `GetStubDecoderRouteInstructions(unsupported_or_invalid_route)`, `FindStubDecoderRouteManifest(unsupported_or_invalid_route)`, plus unknown-instruction, empty-instruction, lowercased-known-opcode, whitespace-padded-known-opcode, and prefixed or suffixed known-opcode lookup exclusion
  - exact negative-path checks now pin `DecodeVop3pStub` / `DecodeMimgTensorStub` / `DecodeVop1Stub` / `DecodeVop3SdstStub` wrong-entrypoint rejection across the full routed seed set, pin both `DecodeStubInstruction()` and all four route-keyed entrypoints on the unsupported seeded slice, pin `DecodeStubInstruction(const StubDecoderRouteInfo&)` normalization for both unsupported and invalid route infos across valid and invalid caller decode hints, pin unknown-op, empty-instruction, lowercased-known-opcode, whitespace-padded-known-opcode, and prefixed or suffixed known-opcode route-keyed parity, and pin null manifest lookups for both `kUnsupported` and invalid route enums
