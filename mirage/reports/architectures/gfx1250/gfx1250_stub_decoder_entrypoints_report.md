...
- Route manifests:
  ...
  - exact route-info negative-path checks now pin supported synthetic `StubDecoderRouteInfo` inputs with invalid caller decode hints across routed, empty-name, synthetic-unknown, and clean unsupported-seeded (`V_CVT_SCALEF32_PK8_FP8_F32`) cases, while lowercased / leading-whitespace near-miss known opcodes continue to preserve caller route metadata and return an empty `kUnknown` structural decode through the route-specific entrypoint name
