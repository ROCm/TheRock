...
int main() {
  ...
  const std::uint32_t typed_buffer_low_component_dst_sel_word3 =
      (4u << 0) | (5u << 3) | (6u << 6) | (7u << 9);

  LinearExecutionMemory typed_buffer_low_component_memory(0x600, 0);
  if (!Expect(WriteU8(&typed_buffer_low_component_memory, 0x380u, 0x5au),
              "expected typed buffer x seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x384u, 0x6bu),
              "expected typed buffer x seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x388u, 0x7cu),
              "expected typed buffer x seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x3c0u, 0x09u),
              "expected typed buffer xy seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x3c1u, 0x0au),
              "expected typed buffer xy seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x3c4u, 0x19u),
              "expected typed buffer xy seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x3c5u, 0x1au),
              "expected typed buffer xy seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x3c8u, 0x29u),
              "expected typed buffer xy seed write") ||
      !Expect(WriteU8(&typed_buffer_low_component_memory, 0x3c9u, 0x2au),
              "expected typed buffer xy seed write") ||
      !Expect(WriteU16(&typed_buffer_low_component_memory, 0x400u, 0x1234u),
              "expected typed buffer d16 x seed write") ||
      !Expect(WriteU16(&typed_buffer_low_component_memory, 0x404u, 0x5678u),
              "expected typed buffer d16 x seed write") ||
      !Expect(WriteU16(&typed_buffer_low_component_memory, 0x408u, 0x9abcu),
              "expected typed buffer d16 x seed write")) {
    return 1;
  }

  WaveExecutionState typed_buffer_low_component_state{};
  typed_buffer_low_component_state.exec_mask = 0b1011ULL;
  typed_buffer_low_component_state.sgprs[60] = 0x380u;
  typed_buffer_low_component_state.sgprs[61] = 0u;
  typed_buffer_low_component_state.sgprs[62] = 0x80u;
  typed_buffer_low_component_state.sgprs[63] =
      typed_buffer_low_component_dst_sel_word3;
  typed_buffer_low_component_state.sgprs[64] = 0x3c0u;
  typed_buffer_low_component_state.sgprs[65] = 0u;
  typed_buffer_low_component_state.sgprs[66] = 0x80u;
  typed_buffer_low_component_state.sgprs[67] =
      typed_buffer_low_component_dst_sel_word3;
  typed_buffer_low_component_state.sgprs[68] = 0x400u;
  typed_buffer_low_component_state.sgprs[69] = 0u;
  typed_buffer_low_component_state.sgprs[70] = 0x80u;
  typed_buffer_low_component_state.sgprs[71] =
      typed_buffer_low_component_dst_sel_word3;
  ...
  typed_buffer_low_component_state.vgprs[12][2] = 0xdeadbeefu;
  ...
  const std::vector<DecodedInstruction> typed_buffer_low_component_program = {
      DecodedInstruction::SevenOperand("TBUFFER_LOAD_FORMAT_X",
                                       ...
                                       InstructionOperand::Imm32(4)),
      DecodedInstruction::SevenOperand("TBUFFER_STORE_FORMAT_X",
                                       InstructionOperand::Vgpr(20),
                                       InstructionOperand::Vgpr(0),
                                       InstructionOperand::Sgpr(60),
                                       InstructionOperand::Imm32(0),
                                       ...
                                       InstructionOperand::Imm32(10),
                                       InstructionOperand::Imm32(4)),
      DecodedInstruction::SevenOperand("TBUFFER_LOAD_FORMAT_XY",
                                       InstructionOperand::Vgpr(11),
                                       InstructionOperand::Vgpr(1),
                                       InstructionOperand::Sgpr(64),
                                       InstructionOperand::Imm32(0),
                                       ...
                                       InstructionOperand::Imm32(10),
                                       InstructionOperand::Imm32(4)),
      DecodedInstruction::SevenOperand("TBUFFER_STORE_FORMAT_XY",
                                       InstructionOperand::Vgpr(21),
                                       InstructionOperand::Vgpr(1),
                                       InstructionOperand::Sgpr(64),
                                       InstructionOperand::Imm32(0),
                                       ...
                                       InstructionOperand::Imm32(10),
                                       InstructionOperand::Imm32(4)),
      DecodedInstruction::SevenOperand("TBUFFER_LOAD_FORMAT_D16_X",
                                       InstructionOperand::Vgpr(13),
                                       InstructionOperand::Vgpr(2),
                                       InstructionOperand::Sgpr(68),
                                       InstructionOperand::Imm32(0),
                                       InstructionOperand::Imm32(0),
                                       InstructionOperand::Imm32(5),
                                       InstructionOperand::Imm32(4)),
      DecodedInstruction::SevenOperand("TBUFFER_STORE_FORMAT_D16_X",
                                       InstructionOperand::Vgpr(23),
                                       InstructionOperand::Vgpr(2),
                                       InstructionOperand::Sgpr(68),
                                       InstructionOperand::Imm32(0),
  ...
  std::vector<CompiledInstruction> compiled_typed_buffer_low_component_program;
  if (!Expect(
          interpreter.CompileProgram(typed_buffer_low_component_program,
                                     &compiled_typed_buffer_low_component_program,
                                     &error_message),
          error_message.c_str())) {
    return 1;
  }

  const LinearExecutionMemory initial_typed_buffer_low_component_memory =
      typed_buffer_low_component_memory;
  const WaveExecutionState initial_typed_buffer_low_component_state =
      typed_buffer_low_component_state;
  const auto validate_typed_buffer_low_component =
    ...
    return Expect(state.halted,
           ...
           expect_lane_values(
               ...
               (std::string(mode) + " typed buffer x load result").c_str()) &&
           expect_lane_values(
               11, {0x09u, 0x19u, 0x29u},
               (std::string(mode) + " typed buffer xy low load result").c_str()) &&
           expect_lane_values(
               12, {0x0au, 0x1au, 0x2au},
               (std::string(mode) + " typed buffer xy high load result").c_str()) &&
           expect_lane_values(
           ...
           Expect(typed_buffer_low_component_byte == 0x6bu,
                  (std::string(mode) + " typed buffer x store lane 0 result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3a4u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer x store lane 1 read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0x7cu,
                  (std::string(mode) + " typed buffer x store lane 1 result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3a8u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer x store lane 3 read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0x8du,
                  (std::string(mode) + " typed buffer x store lane 3 result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3e0u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer xy store lane 0 low read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0xcdu,
                  (std::string(mode) + " typed buffer xy store lane 0 low result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3e1u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer xy store lane 0 high read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0xefu,
                  (std::string(mode) + " typed buffer xy store lane 0 high result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3e4u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer xy store lane 1 low read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0x34u,
                  (std::string(mode) + " typed buffer xy store lane 1 low result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3e5u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer xy store lane 1 high read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0x56u,
                  (std::string(mode) + " typed buffer xy store lane 1 high result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3e8u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer xy store lane 3 low read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0x78u,
                  (std::string(mode) + " typed buffer xy store lane 3 low result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x3e9u, &typed_buffer_low_component_byte),
                  (std::string(mode) + " typed buffer xy store lane 3 high read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_byte == 0x9au,
                  (std::string(mode) + " typed buffer xy store lane 3 high result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x420u, &typed_buffer_low_component_short),
                  (std::string(mode) + " typed buffer d16 x store lane 0 read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_short == 0x5678u,
                  (std::string(mode) + " typed buffer d16 x store lane 0 result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x424u, &typed_buffer_low_component_short),
                  (std::string(mode) + " typed buffer d16 x store lane 1 read")
                      .c_str()) &&
           Expect(typed_buffer_low_component_short == 0xabcdu,
                  (std::string(mode) + " typed buffer d16 x store lane 1 result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x428u, &typed_buffer_low_component_short),
                  (std::string(mode) + " typed buffer d16 x store lane 3 read")
                      .c_str()) &&
  ...
  if (!Expect(interpreter.ExecuteProgram(typed_buffer_low_component_program,
                                         ...
                                         &error_message),
              error_message.c_str()) ||
      !validate_typed_buffer_low_component(typed_buffer_low_component_state,
                                           typed_buffer_low_component_memory,
                                           "decoded")) {
    return 1;
  }

  LinearExecutionMemory compiled_typed_buffer_low_component_memory =
      initial_typed_buffer_low_component_memory;
  WaveExecutionState compiled_typed_buffer_low_component_state =
      initial_typed_buffer_low_component_state;
  if (!Expect(interpreter.ExecuteProgram(
                  compiled_typed_buffer_low_component_program,
                  &compiled_typed_buffer_low_component_state,
                  &compiled_typed_buffer_low_component_memory,
                  &error_message),
              error_message.c_str()) ||
      !validate_typed_buffer_low_component(
          compiled_typed_buffer_low_component_state,
          compiled_typed_buffer_low_component_memory, "compiled")) {
    return 1;
  }
  }
  ...
  auto make_ds_access_state = []() {
    ...
    state.vgprs[31][0] = 0x01020304u;
    state.vgprs[31][1] = 0x11121314u;
    state.vgprs[31][2] = 0x21222324u;
    state.vgprs[31][3] = 0x31323334u;
  ...
  auto validate_ds_access_state = [&](const WaveExecutionState& state,
    ...
    if (!Expect(state.halted, "expected ds access program to halt") ||
        !Expect(state.exec_mask == 0b1011ULL,
                "expected ds access to preserve exec") ||
        !Expect(state.sgprs[0] == 0x13572468u,
                "expected ds access to preserve sgprs") ||
        !expect_active_vgpr(0, 0u, 4u, 8u,
                            "expected ds access to preserve base addresses") ||
        !expect_active_vgpr(1, 0x80ff1122u, 0x7f008877u, 0x1234fedcu,
                            "expected ds access to preserve subword sources") ||
        !expect_active_vgpr(2, 32u, 64u, 96u,
                            "expected ds access to preserve read2 addresses") ||
        !expect_active_vgpr(5, 128u, 160u, 192u,
                            "expected ds access to preserve read2st64 addresses") ||
        !expect_active_vgpr(18, 224u, 228u, 232u,
                            "expected ds access to preserve d16 byte addresses") ||
        !expect_active_vgpr(20, 0x00000180u, 0x1234007fu, 0xffff00ffu,
                            "expected ds access to preserve d16 byte sources") ||
        !expect_active_vgpr(25, 256u, 260u, 264u,
                            "expected ds access to preserve d16 half addresses") ||
        !expect_active_vgpr(27, 0xaaaa5566u, 0xbbbb1234u, 0xccccabcdu,
                            "expected ds access to preserve d16 half sources") ||
        !Expect(state.vgprs[31][0] == 0x01020304u &&
                    state.vgprs[31][1] == 0x11121314u &&
                    state.vgprs[31][2] == 0x21222324u &&
                    state.vgprs[31][3] == 0x31323334u,
                "expected ds access to preserve unrelated vgprs")) {
      std::cerr << mode << '\n';
      return false;
    }
    ...
    if (!Expect(state.vgprs[10][2] == 0xdeadbeefu,
      ...
      return false;
        ...
        !Expect((std::memcpy(&value, state.lds_bytes.data() + 256, sizeof(value)), value) ==
	        ...
	        !Expect((std::memcpy(&value, state.lds_bytes.data() + 0x300u, sizeof(value)), value) ==
	                    0x11223344u,
	                "expected ds access to preserve unrelated lds")) {
	      std::cerr << mode << '\n';
	      return false;
	    }
    return true;
  ...
  auto make_ds_multi_dword_access_state = []() {
    ...
    const std::uint32_t untouched_lds = 0x11223344u;
    std::memcpy(state.lds_bytes.data() + 0x200u, &untouched_lds,
                sizeof(untouched_lds));
    return state;
  };
  auto validate_ds_multi_dword_access_state =
      [&](const WaveExecutionState& state, const char* mode) {
        ...
        if (!Expect(state.halted,
            ...
            !Expect(state.exec_mask == 0b1011ULL,
                    "expected ds multi-dword access to preserve exec") ||
            !Expect(state.sgprs[0] == 0x13572468u,
                    "expected ds multi-dword access to preserve sgprs") ||
            !expect_active_vgpr(0, 0u, 16u, 32u,
                                "expected ds multi-dword access to preserve b96 addresses") ||
            !expect_active_vgpr(1, 0x11111111u, 0x44444444u, 0x77777777u,
                                "expected ds multi-dword access to preserve b96 source dword 0") ||
            !expect_active_vgpr(2, 0x22222222u, 0x55555555u, 0x88888888u,
                                "expected ds multi-dword access to preserve b96 source dword 1") ||
            !expect_active_vgpr(3, 0x33333333u, 0x66666666u, 0x99999999u,
                                "expected ds multi-dword access to preserve b96 source dword 2") ||
            !expect_active_vgpr(4, 64u, 96u, 128u,
                                "expected ds multi-dword access to preserve b128 addresses") ||
            !expect_active_vgpr(5, 0x0a0b0c0du, 0x10203040u, 0x55667788u,
                                "expected ds multi-dword access to preserve b128 source dword 0") ||
            !expect_active_vgpr(6, 0x1a1b1c1du, 0x20304050u, 0x66778899u,
                                "expected ds multi-dword access to preserve b128 source dword 1") ||
            !expect_active_vgpr(7, 0x2a2b2c2du, 0x30405060u, 0x778899aau,
                                "expected ds multi-dword access to preserve b128 source dword 2") ||
            !expect_active_vgpr(8, 0x3a3b3c3du, 0x40506070u, 0x8899aabbu,
                                "expected ds multi-dword access to preserve b128 source dword 3") ||
            !Expect(state.vgprs[24][0] == 0x01020304u &&
                        state.vgprs[24][1] == 0x11121314u &&
                        state.vgprs[24][2] == 0x21222324u &&
                        state.vgprs[24][3] == 0x31323334u,
                    "expected ds multi-dword access to preserve unrelated vgprs")) {
          std::cerr << mode << '\n';
          return false;
        }
        ...
        const auto expect_lds_value = [&](std::uint64_t address,
          ...
          return Expect(value == expected, label);
            ...
            !expect_lds_value(0x200u, 0x11223344u,
                              "expected ds multi-dword access to preserve unrelated lds")) {
	          std::cerr << mode << '\n';
	          return false;
	        }
        return true;
  ...
  if (!Expect(WriteU8(&buffer_format_memory, 0x100u, 0x7au),
      ...
      !Expect(WriteU16(&buffer_format_memory, 0x282u, 0x3c00u),
              "expected buffer format d16 xyzw seed write") ||
      !Expect(WriteU16(&buffer_format_memory, 0x284u, 0x4000u),
              "expected buffer format d16 xyzw seed write") ||
  ...
  buffer_format_state.sgprs[44] = 0x280u;
  buffer_format_state.sgprs[45] = 0u;
  ...
  {
  auto make_buffer_format_high_component_word3 = [](std::uint32_t data_format,
                                                    std::uint32_t num_format) {
    return (4u << 0) | (5u << 3) | (6u << 6) | (7u << 9) |
           (data_format << 12) | (num_format << 19);
  };
  ...
  if (!Expect(WriteU8(&buffer_format_high_component_memory, 0x140u, 0x01u),
      ...
      !Expect(WriteU8(&buffer_format_high_component_memory, 0x14bu, 0x24u),
              "expected buffer format high-component xyzw seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x180u, 0x3fc00000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x184u, 0xc0000000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x188u, 0x3e800000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x18cu, 0x40200000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x190u, 0xbf800000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x194u, 0x40400000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x1b0u, 0x40a00000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x1b4u, 0xbf000000u),
              "expected buffer format high-component xyz seed write") ||
      !Expect(buffer_format_high_component_memory.WriteU32(0x1b8u, 0x41000000u),
              "expected buffer format high-component xyz seed write") ||
  ...
  buffer_format_high_component_state.sgprs[27] =
      make_buffer_format_high_component_word3(10u, 4u);
  ...
  buffer_format_high_component_state.sgprs[31] =
      make_buffer_format_high_component_word3(13u, 7u);
  ...
  buffer_format_high_component_state.sgprs[35] =
      make_buffer_format_high_component_word3(5u, 4u);
  ...
  buffer_format_high_component_state.sgprs[39] =
      make_buffer_format_high_component_word3(12u, 7u);
  ...
  if (!Expect(interpreter.CompileProgram(
                  buffer_format_high_component_program,
                  &compiled_buffer_format_high_component_program, &error_message),
              error_message.c_str())) {
    return 1;
  }
  ...
  const auto validate_buffer_format_high_component =
    ...
    return Expect(state.halted,
           ...
           expect_lane_values(
               81, {0x40003c00u, 0x38003400u, 0x44004200u},
               (std::string(mode) + " buffer format d16 xyz packed xy result")
                   .c_str()) &&
           expect_lane_values(
               82, {0x0000c000u, 0x00003c00u, 0x00004600u},
           ...
           expect_lane_values(
               84, {0x3c003800u, 0x38003400u, 0x44004200u},
               (std::string(mode) + " buffer format d16 xyzw packed xy result")
                   .c_str()) &&
           expect_lane_values(
               85, {0xc0004000u, 0x40003c00u, 0x48004600u},
           ...
           Expect(byte_value == 0x05u,
                  (std::string(mode) + " buffer format xyzw store lane 0 x result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x161u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 0 y read")
                      .c_str()) &&
           Expect(byte_value == 0x06u,
                  (std::string(mode) + " buffer format xyzw store lane 0 y result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x162u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 0 z read")
                      .c_str()) &&
           Expect(byte_value == 0x07u,
                  (std::string(mode) + " buffer format xyzw store lane 0 z result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x163u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 0 w read")
                      .c_str()) &&
           Expect(byte_value == 0x08u,
                  (std::string(mode) + " buffer format xyzw store lane 0 w result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x164u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 1 x read")
                      .c_str()) &&
           Expect(byte_value == 0x15u,
                  (std::string(mode) + " buffer format xyzw store lane 1 x result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x165u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 1 y read")
                      .c_str()) &&
           Expect(byte_value == 0x16u,
                  (std::string(mode) + " buffer format xyzw store lane 1 y result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x166u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 1 z read")
                      .c_str()) &&
           Expect(byte_value == 0x17u,
                  (std::string(mode) + " buffer format xyzw store lane 1 z result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x167u, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 1 w read")
                      .c_str()) &&
           Expect(byte_value == 0x18u,
                  (std::string(mode) + " buffer format xyzw store lane 1 w result")
           ...
           Expect(byte_value == 0x26u,
                  (std::string(mode) + " buffer format xyzw store lane 3 y result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x16au, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 3 z read")
                      .c_str()) &&
           Expect(byte_value == 0x27u,
                  (std::string(mode) + " buffer format xyzw store lane 3 z result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x16bu, &byte_value),
                  (std::string(mode) + " buffer format xyzw store lane 3 w read")
                      .c_str()) &&
           Expect(byte_value == 0x28u,
                  (std::string(mode) + " buffer format xyzw store lane 3 w result")
           ...
           Expect(dword_value == 0x40400000u,
                  (std::string(mode) + " buffer format xyz store lane 1 z result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x1d0u, &dword_value),
                  (std::string(mode) + " buffer format xyz store lane 3 x read")
                      .c_str()) &&
           Expect(dword_value == 0x40c00000u,
                  (std::string(mode) + " buffer format xyz store lane 3 x result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x1d4u, &dword_value),
                  (std::string(mode) + " buffer format xyz store lane 3 y read")
                      .c_str()) &&
           Expect(dword_value == 0xbf000000u,
                  (std::string(mode) + " buffer format xyz store lane 3 y result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x1d8u, &dword_value),
                  (std::string(mode) + " buffer format xyz store lane 3 z read")
                      .c_str()) &&
           Expect(dword_value == 0x41000000u,
                  (std::string(mode) + " buffer format xyz store lane 3 z result")
           ...
           Expect(short_value == 0x3400u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 0 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2a2u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 0 y read")
                      .c_str()) &&
           Expect(short_value == 0x3800u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 0 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2a4u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 0 z read")
                      .c_str()) &&
           Expect(short_value == 0x3c00u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 0 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2a6u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 0 w read")
                      .c_str()) &&
           Expect(short_value == 0x4000u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 0 w result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2a8u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 1 x read")
                      .c_str()) &&
           Expect(short_value == 0x3800u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 1 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2aau, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 1 y read")
                      .c_str()) &&
           Expect(short_value == 0x3c00u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 1 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2acu, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 1 z read")
                      .c_str()) &&
           Expect(short_value == 0x4000u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 1 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2aeu, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 1 w read")
                      .c_str()) &&
           Expect(short_value == 0x4400u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 1 w result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2b0u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 3 x read")
                      .c_str()) &&
           Expect(short_value == 0x4200u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 3 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2b2u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 3 y read")
                      .c_str()) &&
           Expect(short_value == 0x4400u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 3 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2b4u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 3 z read")
                      .c_str()) &&
           Expect(short_value == 0x4600u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 3 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x2b6u, &short_value),
                  (std::string(mode) + " buffer format d16 xyzw store lane 3 w read")
                      .c_str()) &&
           Expect(short_value == 0x4800u,
                  (std::string(mode) +
                   " buffer format d16 xyzw store lane 3 w result")
                      .c_str()) &&
           ...
           Expect(byte_value == 0u,
                  (std::string(mode) + " inactive buffer format xyzw store result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x1ecu, &short_value),
                  (std::string(mode) + " inactive buffer format d16 xy store read")
                      .c_str()) &&
           Expect(short_value == 0u,
                  (std::string(mode) + " inactive buffer format d16 xy store result")
  ...
  if (!Expect(interpreter.ExecuteProgram(buffer_format_high_component_program,
      ...
      !validate_buffer_format_high_component(
          buffer_format_high_component_state,
          buffer_format_high_component_memory, "decoded")) {
    return 1;
  }
  ...
  if (!Expect(WriteU8(&typed_buffer_memory, 0x280u, 0x11u),
      ...
      !Expect(WriteU8(&typed_buffer_memory, 0x283u, 0x44u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x284u, 0x51u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x285u, 0x62u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x286u, 0x73u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x287u, 0x84u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x288u, 0x91u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x289u, 0xa2u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x28au, 0xb3u),
              "expected typed buffer xyzw seed write") ||
      !Expect(WriteU8(&typed_buffer_memory, 0x28bu, 0xc4u),
              "expected typed buffer xyzw seed write") ||
      ...
      !Expect(typed_buffer_memory.WriteU32(0x2c8u, 0x40400000u),
              "expected typed buffer xyz seed write") ||
      !Expect(typed_buffer_memory.WriteU32(0x2ccu, 0x40400000u),
              "expected typed buffer xyz seed write") ||
      !Expect(typed_buffer_memory.WriteU32(0x2d0u, 0xc0200000u),
              "expected typed buffer xyz seed write") ||
      !Expect(typed_buffer_memory.WriteU32(0x2d4u, 0x3d800000u),
              "expected typed buffer xyz seed write") ||
      !Expect(typed_buffer_memory.WriteU32(0x2f0u, 0x40a00000u),
              "expected typed buffer xyz seed write") ||
      !Expect(typed_buffer_memory.WriteU32(0x2f4u, 0xbf000000u),
              "expected typed buffer xyz seed write") ||
      !Expect(typed_buffer_memory.WriteU32(0x2f8u, 0x41000000u),
              "expected typed buffer xyz seed write") ||
      ...
      !Expect(WriteU16(&typed_buffer_memory, 0x302u, 0x0304u),
              "expected typed buffer d16 xy seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x304u, 0x0506u),
              "expected typed buffer d16 xy seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x306u, 0x0708u),
              "expected typed buffer d16 xy seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x308u, 0x090au),
              "expected typed buffer d16 xy seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x30au, 0x0b0cu),
              "expected typed buffer d16 xy seed write") ||
      ...
      !Expect(WriteU16(&typed_buffer_memory, 0x344u, 0x4000u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x348u, 0x4200u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x34au, 0x4400u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x34cu, 0x4600u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x350u, 0x4800u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x352u, 0x4a00u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x354u, 0x4c00u),
              "expected typed buffer d16 xyz seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x380u, 0x3800u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x382u, 0x3c00u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x384u, 0x4000u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x386u, 0xc000u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x388u, 0x4200u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x38au, 0x4400u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x38cu, 0x4600u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x38eu, 0x4800u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x390u, 0x4a00u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x392u, 0x4c00u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x394u, 0x4e00u),
              "expected typed buffer d16 xyzw seed write") ||
      !Expect(WriteU16(&typed_buffer_memory, 0x396u, 0x5000u),
              "expected typed buffer d16 xyzw seed write")) {
    return 1;
  }

  WaveExecutionState typed_buffer_state{};
  typed_buffer_state.exec_mask = 0b1011ULL;
  ...
  typed_buffer_state.sgprs[60] = 0x380u;
  typed_buffer_state.sgprs[61] = 0u;
  typed_buffer_state.sgprs[62] = 0x80u;
  typed_buffer_state.sgprs[63] = typed_buffer_dst_sel_word3;
  typed_buffer_state.vgprs[0][0] = 0u;
  typed_buffer_state.vgprs[0][1] = 4u;
  typed_buffer_state.vgprs[0][2] = 0x0cu;
  typed_buffer_state.vgprs[0][3] = 8u;
  typed_buffer_state.vgprs[1][0] = 0u;
  typed_buffer_state.vgprs[1][1] = 0x0cu;
  typed_buffer_state.vgprs[1][2] = 0x2cu;
  typed_buffer_state.vgprs[1][3] = 0x30u;
  typed_buffer_state.vgprs[2][0] = 0u;
  typed_buffer_state.vgprs[2][1] = 4u;
  typed_buffer_state.vgprs[2][2] = 0x0cu;
  typed_buffer_state.vgprs[2][3] = 8u;
  typed_buffer_state.vgprs[3][0] = 0u;
  typed_buffer_state.vgprs[3][1] = 8u;
  typed_buffer_state.vgprs[3][2] = 0x18u;
  typed_buffer_state.vgprs[3][3] = 0x10u;
  typed_buffer_state.vgprs[4][0] = 0u;
  typed_buffer_state.vgprs[4][1] = 8u;
  typed_buffer_state.vgprs[4][2] = 0x18u;
  typed_buffer_state.vgprs[4][3] = 0x10u;
  typed_buffer_state.vgprs[90][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[91][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[92][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[93][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[94][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[95][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[96][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[97][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[98][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[99][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[104][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[105][2] = 0xdeadbeefu;
  typed_buffer_state.vgprs[100][0] = 0xa1u;
  typed_buffer_state.vgprs[100][1] = 0xa5u;
  typed_buffer_state.vgprs[100][3] = 0xa9u;
  typed_buffer_state.vgprs[101][0] = 0xb2u;
  typed_buffer_state.vgprs[101][1] = 0xb6u;
  typed_buffer_state.vgprs[101][3] = 0xbau;
  typed_buffer_state.vgprs[102][0] = 0xc3u;
  typed_buffer_state.vgprs[102][1] = 0xc7u;
  typed_buffer_state.vgprs[102][3] = 0xcbu;
  typed_buffer_state.vgprs[103][0] = 0xd4u;
  typed_buffer_state.vgprs[103][1] = 0xd8u;
  typed_buffer_state.vgprs[103][3] = 0xdcu;
  typed_buffer_state.vgprs[110][0] = 0x3f000000u;
  typed_buffer_state.vgprs[110][1] = 0x40400000u;
  typed_buffer_state.vgprs[110][3] = 0x40a00000u;
  typed_buffer_state.vgprs[111][0] = 0xbf800000u;
  typed_buffer_state.vgprs[111][1] = 0xc0400000u;
  typed_buffer_state.vgprs[111][3] = 0xbf000000u;
  typed_buffer_state.vgprs[112][0] = 0x40200000u;
  typed_buffer_state.vgprs[112][1] = 0x3f800000u;
  typed_buffer_state.vgprs[112][3] = 0x41200000u;
  typed_buffer_state.vgprs[120][0] = 0x03040102u;
  typed_buffer_state.vgprs[120][1] = 0x07080506u;
  typed_buffer_state.vgprs[120][3] = 0x0b0c090au;
  typed_buffer_state.vgprs[121][0] = 0x3c003800u;
  typed_buffer_state.vgprs[121][1] = 0x44004200u;
  typed_buffer_state.vgprs[121][3] = 0x4a004800u;
  typed_buffer_state.vgprs[122][0] = 0x00004000u;
  typed_buffer_state.vgprs[122][1] = 0x00004600u;
  typed_buffer_state.vgprs[122][3] = 0x00004c00u;
  typed_buffer_state.vgprs[123][0] = 0x38003400u;
  typed_buffer_state.vgprs[123][1] = 0x42004000u;
  typed_buffer_state.vgprs[123][3] = 0x4a004800u;
  ...
  const auto validate_typed_buffer =
    ...
    return Expect(state.halted,
           ...
           expect_lane_values(
               91, {0x22u, 0x62u, 0xa2u},
               (std::string(mode) + " typed buffer xyzw load y result").c_str()) &&
           expect_lane_values(
               92, {0x33u, 0x73u, 0xb3u},
               (std::string(mode) + " typed buffer xyzw load z result").c_str()) &&
           expect_lane_values(
               93, {0x44u, 0x84u, 0xc4u},
               (std::string(mode) + " typed buffer xyzw load w result").c_str()) &&
           expect_lane_values(
               94, {0x3f800000u, 0x40400000u, 0x40a00000u},
               (std::string(mode) + " typed buffer xyz load x result").c_str()) &&
           expect_lane_values(
               95, {0xbf400000u, 0xc0200000u, 0xbf000000u},
               (std::string(mode) + " typed buffer xyz load y result").c_str()) &&
           expect_lane_values(
               96, {0x40400000u, 0x3d800000u, 0x41000000u},
               (std::string(mode) + " typed buffer xyz load z result").c_str()) &&
           expect_lane_values(
               97, {0x03040102u, 0x07080506u, 0x0b0c090au},
               (std::string(mode) + " typed buffer d16 xy load result").c_str()) &&
           expect_lane_values(
               98, {0x3c003800u, 0x44004200u, 0x4a004800u},
               (std::string(mode) + " typed buffer d16 xyz packed xy result")
                   .c_str()) &&
           expect_lane_values(
               99, {0x00004000u, 0x00004600u, 0x00004c00u},
               (std::string(mode) + " typed buffer d16 xyz z result").c_str()) &&
           expect_lane_values(
               104, {0x3c003800u, 0x44004200u, 0x4c004a00u},
               (std::string(mode) + " typed buffer d16 xyzw packed xy result")
                   .c_str()) &&
           expect_lane_values(
           ...
           Expect(ReadU8(memory, 0x2a0u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 0 x read")
                      .c_str()) &&
           Expect(byte_value == 0xa1u,
                  (std::string(mode) + " typed buffer xyzw store lane 0 x result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a1u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 0 y read")
                      .c_str()) &&
           Expect(byte_value == 0xb2u,
                  (std::string(mode) + " typed buffer xyzw store lane 0 y result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a2u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 0 z read")
                      .c_str()) &&
           Expect(byte_value == 0xc3u,
                  (std::string(mode) + " typed buffer xyzw store lane 0 z result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a3u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 0 w read")
                      .c_str()) &&
           Expect(byte_value == 0xd4u,
                  (std::string(mode) + " typed buffer xyzw store lane 0 w result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a4u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 1 x read")
                      .c_str()) &&
           Expect(byte_value == 0xa5u,
                  (std::string(mode) + " typed buffer xyzw store lane 1 x result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a5u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 1 y read")
                      .c_str()) &&
           Expect(byte_value == 0xb6u,
                  (std::string(mode) + " typed buffer xyzw store lane 1 y result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a6u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 1 z read")
                      .c_str()) &&
           Expect(byte_value == 0xc7u,
                  (std::string(mode) + " typed buffer xyzw store lane 1 z result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a7u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 1 w read")
                      .c_str()) &&
           Expect(byte_value == 0xd8u,
                  (std::string(mode) + " typed buffer xyzw store lane 1 w result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a8u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 3 x read")
                      .c_str()) &&
           Expect(byte_value == 0xa9u,
                  (std::string(mode) + " typed buffer xyzw store lane 3 x result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2a9u, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 3 y read")
                      .c_str()) &&
           Expect(byte_value == 0xbau,
                  (std::string(mode) + " typed buffer xyzw store lane 3 y result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2aau, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 3 z read")
                      .c_str()) &&
           Expect(byte_value == 0xcbu,
                  (std::string(mode) + " typed buffer xyzw store lane 3 z result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2abu, &byte_value),
                  (std::string(mode) + " typed buffer xyzw store lane 3 w read")
                      .c_str()) &&
           Expect(byte_value == 0xdcu,
                  (std::string(mode) + " typed buffer xyzw store lane 3 w result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x2e0u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 0 x read")
                      .c_str()) &&
           Expect(dword_value == 0x3f000000u,
                  (std::string(mode) + " typed buffer xyz store lane 0 x result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x2e4u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 0 y read")
                      .c_str()) &&
           Expect(dword_value == 0xbf800000u,
                  (std::string(mode) + " typed buffer xyz store lane 0 y result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x2e8u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 0 z read")
                      .c_str()) &&
           Expect(dword_value == 0x40200000u,
                  (std::string(mode) + " typed buffer xyz store lane 0 z result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x2ecu, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 1 x read")
                      .c_str()) &&
           Expect(dword_value == 0x40400000u,
                  (std::string(mode) + " typed buffer xyz store lane 1 x result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x2f0u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 1 y read")
                      .c_str()) &&
           Expect(dword_value == 0xc0400000u,
                  (std::string(mode) + " typed buffer xyz store lane 1 y result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x2f4u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 1 z read")
                      .c_str()) &&
           Expect(dword_value == 0x3f800000u,
                  (std::string(mode) + " typed buffer xyz store lane 1 z result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x310u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 3 x read")
                      .c_str()) &&
           Expect(dword_value == 0x40a00000u,
                  (std::string(mode) + " typed buffer xyz store lane 3 x result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x314u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 3 y read")
                      .c_str()) &&
           Expect(dword_value == 0xbf000000u,
                  (std::string(mode) + " typed buffer xyz store lane 3 y result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x318u, &dword_value),
                  (std::string(mode) + " typed buffer xyz store lane 3 z read")
                      .c_str()) &&
           Expect(dword_value == 0x41200000u,
                  (std::string(mode) + " typed buffer xyz store lane 3 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x320u, &short_value),
                  (std::string(mode) + " typed buffer d16 xy store lane 0 x read")
                      .c_str()) &&
           Expect(short_value == 0x0102u,
                  (std::string(mode) + " typed buffer d16 xy store lane 0 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x322u, &short_value),
                  (std::string(mode) + " typed buffer d16 xy store lane 0 y read")
                      .c_str()) &&
           Expect(short_value == 0x0304u,
                  (std::string(mode) + " typed buffer d16 xy store lane 0 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x324u, &short_value),
                  (std::string(mode) + " typed buffer d16 xy store lane 1 x read")
                      .c_str()) &&
           Expect(short_value == 0x0506u,
                  (std::string(mode) + " typed buffer d16 xy store lane 1 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x326u, &short_value),
                  (std::string(mode) + " typed buffer d16 xy store lane 1 y read")
                      .c_str()) &&
           Expect(short_value == 0x0708u,
                  (std::string(mode) + " typed buffer d16 xy store lane 1 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x328u, &short_value),
                  (std::string(mode) + " typed buffer d16 xy store lane 3 x read")
                      .c_str()) &&
           Expect(short_value == 0x090au,
                  (std::string(mode) + " typed buffer d16 xy store lane 3 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x32au, &short_value),
                  (std::string(mode) + " typed buffer d16 xy store lane 3 y read")
                      .c_str()) &&
           Expect(short_value == 0x0b0cu,
                  (std::string(mode) + " typed buffer d16 xy store lane 3 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x360u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 0 x read")
                      .c_str()) &&
           Expect(short_value == 0x3800u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 0 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x362u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 0 y read")
                      .c_str()) &&
           Expect(short_value == 0x3c00u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 0 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x364u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 0 z read")
                      .c_str()) &&
           Expect(short_value == 0x4000u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 0 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x368u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 1 x read")
                      .c_str()) &&
           Expect(short_value == 0x4200u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 1 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x36au, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 1 y read")
                      .c_str()) &&
           Expect(short_value == 0x4400u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 1 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x36cu, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 1 z read")
                      .c_str()) &&
           Expect(short_value == 0x4600u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 1 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x370u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 3 x read")
                      .c_str()) &&
           Expect(short_value == 0x4800u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 3 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x372u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 3 y read")
                      .c_str()) &&
           Expect(short_value == 0x4a00u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 3 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x374u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyz store lane 3 z read")
                      .c_str()) &&
           Expect(short_value == 0x4c00u,
                  (std::string(mode) + " typed buffer d16 xyz store lane 3 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3a0u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 x read")
                      .c_str()) &&
           Expect(short_value == 0x3400u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3a2u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 y read")
                      .c_str()) &&
           Expect(short_value == 0x3800u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3a4u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 z read")
                      .c_str()) &&
           Expect(short_value == 0x3c00u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3a6u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 w read")
                      .c_str()) &&
           Expect(short_value == 0x4000u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 0 w result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3a8u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 x read")
                      .c_str()) &&
           Expect(short_value == 0x4000u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3aau, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 y read")
                      .c_str()) &&
           Expect(short_value == 0x4200u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3acu, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 z read")
                      .c_str()) &&
           Expect(short_value == 0x4400u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3aeu, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 w read")
                      .c_str()) &&
           Expect(short_value == 0x4600u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 1 w result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3b0u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 x read")
                      .c_str()) &&
           Expect(short_value == 0x4800u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 x result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3b2u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 y read")
                      .c_str()) &&
           Expect(short_value == 0x4a00u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 y result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3b4u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 z read")
                      .c_str()) &&
           Expect(short_value == 0x4c00u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 z result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3b6u, &short_value),
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 w read")
                      .c_str()) &&
           Expect(short_value == 0x4e00u,
                  (std::string(mode) + " typed buffer d16 xyzw store lane 3 w result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x2acu, &byte_value),
                  (std::string(mode) + " inactive typed buffer xyzw store read")
                      .c_str()) &&
           Expect(byte_value == 0u,
                  (std::string(mode) + " inactive typed buffer xyzw store result")
                      .c_str()) &&
           Expect(memory.ReadU32(0x30cu, &dword_value),
                  (std::string(mode) + " inactive typed buffer xyz store read")
                      .c_str()) &&
           Expect(dword_value == 0u,
                  (std::string(mode) + " inactive typed buffer xyz store result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x32cu, &short_value),
                  (std::string(mode) + " inactive typed buffer d16 xy store read")
                      .c_str()) &&
           Expect(short_value == 0u,
                  (std::string(mode) + " inactive typed buffer d16 xy store result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x378u, &short_value),
                  (std::string(mode) + " inactive typed buffer d16 xyz store read")
                      .c_str()) &&
           Expect(short_value == 0u,
                  (std::string(mode) + " inactive typed buffer d16 xyz store result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x3b8u, &short_value),
                  (std::string(mode) + " inactive typed buffer d16 xyzw store read")
                      .c_str()) &&
           Expect(short_value == 0u,
                  (std::string(mode) +
  ...
  if (!Expect(interpreter.ExecuteProgram(typed_buffer_program,
                                         ...
                                         &error_message),
              error_message.c_str()) ||
      !validate_typed_buffer(typed_buffer_state, typed_buffer_memory,
                             "decoded")) {
    return 1;
  }
  ...
  if (!Expect(interpreter.ExecuteProgram(compiled_typed_buffer_program,
                                         ...
                                         &error_message),
              error_message.c_str()) ||
      !validate_typed_buffer(compiled_typed_buffer_state,
                             compiled_typed_buffer_memory, "compiled")) {
    return 1;
  }
  }
  ...
  const auto seed_subword_memory = [](LinearExecutionMemory* memory) -> bool {
    return memory != nullptr && WriteU8(memory, 0x600, 0x7au) &&
           WriteU8(memory, 0x700, 0x12u) &&
           WriteU8(memory, 0x800, 0xffu) &&
           WriteU8(memory, 0x610, 0x80u) &&
           WriteU8(memory, 0x710, 0x7fu) &&
           WriteU8(memory, 0x810, 0xfeu) &&
           WriteU16(memory, 0x620, 0x1234u) &&
           WriteU16(memory, 0x720, 0x4567u) &&
           WriteU16(memory, 0x820, 0x89abu) &&
           WriteU16(memory, 0x630, 0x8001u) &&
           WriteU16(memory, 0x730, 0x7ffeu) &&
           WriteU16(memory, 0x830, 0xfffeu) &&
           WriteU8(memory, 0xa20, 0xa5u) &&
           WriteU8(memory, 0xaa0, 0x9bu) &&
           WriteU8(memory, 0xb20, 0xdeu) &&
           WriteU8(memory, 0xa30, 0xf0u) &&
           WriteU8(memory, 0xab0, 0x7eu) &&
           WriteU8(memory, 0xb30, 0x80u) &&
           WriteU16(memory, 0xa40, 0x5678u) &&
           WriteU16(memory, 0xac0, 0x89abu) &&
           WriteU16(memory, 0xb40, 0xcdefu) &&
           WriteU16(memory, 0xa50, 0x8002u) &&
           WriteU16(memory, 0xad0, 0x7ffdu) &&
           WriteU16(memory, 0xb50, 0xfffeu);
  };
  const auto make_subword_state = []() {
    ...
    state.vgprs[0][1] = 0x700;
    state.vgprs[0][3] = 0x800;
    state.vgprs[2][0] = 0x610;
    state.vgprs[2][1] = 0x710;
    state.vgprs[2][3] = 0x810;
    state.vgprs[4][0] = 0x620;
    state.vgprs[4][1] = 0x720;
    state.vgprs[4][3] = 0x820;
    state.vgprs[6][0] = 0x630;
    state.vgprs[6][1] = 0x730;
    state.vgprs[6][3] = 0x830;
    state.vgprs[8][0] = 0x20;
    state.vgprs[8][1] = 0xa0;
    state.vgprs[8][3] = 0x120;
    state.vgprs[10][0] = 0x30;
    state.vgprs[10][1] = 0xb0;
    state.vgprs[10][3] = 0x130;
    state.vgprs[12][0] = 0x40;
    state.vgprs[12][1] = 0xc0;
    state.vgprs[12][3] = 0x140;
    state.vgprs[14][0] = 0x50;
    state.vgprs[14][1] = 0xd0;
    state.vgprs[14][3] = 0x150;
    state.vgprs[16][0] = 0x640;
    state.vgprs[16][1] = 0x740;
    state.vgprs[16][3] = 0x840;
    state.vgprs[18][0] = 0x650;
    state.vgprs[18][1] = 0x750;
    state.vgprs[18][3] = 0x850;
  ...
  const auto validate_subword_memory =
    ...
    return expect_lane_values(
           ...
           Expect(ReadU8(memory, 0x740, &stored_byte),
                  (std::string(mode) + " flat byte store lane 1 read").c_str()) &&
           Expect(stored_byte == 0x88u,
                  (std::string(mode) + " flat byte store lane 1 result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0x840, &stored_byte),
                  (std::string(mode) + " flat byte store lane 3 read").c_str()) &&
           Expect(stored_byte == 0x40u,
                  (std::string(mode) + " flat byte store lane 3 result")
                      .c_str()) &&
           ...
           Expect(ReadU16(memory, 0x750, &stored_short),
                  (std::string(mode) + " flat short store lane 1 read").c_str()) &&
           Expect(stored_short == 0xc3d4u,
                  (std::string(mode) + " flat short store lane 1 result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0x850, &stored_short),
                  (std::string(mode) + " flat short store lane 3 read").c_str()) &&
           Expect(stored_short == 0x7788u,
                  (std::string(mode) + " flat short store lane 3 result")
                      .c_str()) &&
           ...
           Expect(ReadU8(memory, 0xae0, &stored_byte),
                  (std::string(mode) + " global byte store lane 1 read")
                      .c_str()) &&
           Expect(stored_byte == 0xefu,
                  (std::string(mode) + " global byte store lane 1 result")
                      .c_str()) &&
           Expect(ReadU8(memory, 0xb60, &stored_byte),
                  (std::string(mode) + " global byte store lane 3 read")
                      .c_str()) &&
           Expect(stored_byte == 0x67u,
                  (std::string(mode) + " global byte store lane 3 result")
                      .c_str()) &&
           ...
           Expect(ReadU16(memory, 0xaf0, &stored_short),
                  (std::string(mode) + " global short store lane 1 read")
                      .c_str()) &&
           Expect(stored_short == 0x9bdfu,
                  (std::string(mode) + " global short store lane 1 result")
                      .c_str()) &&
           Expect(ReadU16(memory, 0xb70, &stored_short),
                  (std::string(mode) + " global short store lane 3 read")
                      .c_str()) &&
           Expect(stored_short == 0xd00du,
                  (std::string(mode) + " global short store lane 3 result")
                      .c_str());
  };
