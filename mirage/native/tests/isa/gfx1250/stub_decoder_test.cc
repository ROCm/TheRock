...
bool Vop3pTailBatchRouteManifestCountParityMatchesSeedCatalog() {
  const auto seeded_instructions = GetSeededInstructionNames(SeedFamily::kVop3p);
  if (seeded_instructions.size() != 62 ||
      seeded_instructions[50] != "V_WMMA_F32_16X16X64_BF8_BF8_w32" ||
  ...
  if (manifest == nullptr ||
      ...
      manifest->xml_backed_count !=
          CountRouteInfosForRouteWithXmlFlag(StubDecoderRoute::kVop3p, true) ||
      manifest->llvm_only_count !=
          CountRouteInfosForRouteWithXmlFlag(StubDecoderRoute::kVop3p, false) ||
      manifest->target_specific_count !=
          CountRouteInfosForRouteWithTargetSpecificFlag(
              StubDecoderRoute::kVop3p, true)) {
    return false;
  }
...
int main() {
  ...
  for (std::string_view split_token_near_miss_instruction :
       {"V_PK__ADD_BF16",
        ...
        "V_WMMA__F32_16X16X4_F32_w32",
        "V_WMMA_SCALE__F32_16X16X128_F8F6F4",
        "V_WMMA_SCALE__F32_32X16X128_F4_w32"}) {
    const StubDecodedInstruction split_token_near_miss_decode =
        DecodeStubInstruction(split_token_near_miss_instruction);
    if (!Expect(MatchesUnknownDecode(split_token_near_miss_decode,
                                     split_token_near_miss_instruction) &&
                    MatchesUnknownHelperSurface(split_token_near_miss_decode),
                "expected split-token family near-misses to keep exact unknown decode parity")) {
      return 1;
    }
    for (const StubDecoderRouteManifest& manifest :
         GetStubDecoderRouteManifests()) {
      ...
      if (!Expect(MatchesUnknownDecode(via_entrypoint,
                                       split_token_near_miss_instruction) &&
                      MatchesUnknownHelperSurface(via_entrypoint),
                  "expected split-token family near-misses to keep exact route-keyed unknown parity")) {
        return 1;
      }
    }
  }
  for (std::string_view delayed_split_token_near_miss_instruction :
       {"V_PK_ADD__BF16",
        ...
        "V_WMMA_SCALE_F32__32X16X128_F4_w32"}) {
  ...
  const auto fp8_bf8_seeded_instructions =
      GetSeededInstructionNames(SeedFamily::kFp8Bf8);
  if (!Expect(fp8_bf8_seeded_instructions.size() == 87,
  ...
  for (std::size_t i = 0; i < 50; ++i) {
    ...
    const DecoderSeedInfo* seed = FindDecoderSeedInfo(instruction_name);
    if (!Expect(seed != nullptr,
                "expected fp8/bf8 batch instruction to remain present in the seed catalog")) {
      return 1;
    }
    ...
    if (route_info == nullptr) {
      const StubDecodedInstruction decoded = DecodeStubInstruction(instruction_name);
      bool listed_in_any_route = false;
      for (const StubDecoderRouteManifest& manifest :
           GetStubDecoderRouteManifests()) {
        if (route_contains_instruction(manifest.route, instruction_name)) {
          listed_in_any_route = true;
          break;
        }
      }
      if (!Expect(MatchesUnsupportedSeedDecode(decoded, *seed) &&
                      MatchesUnknownHelperSurface(decoded) &&
                      MatchesUnsupportedInstructionDecode(
                          decoded, instruction_name) &&
                      SelectStubDecoderRoute(instruction_name) ==
                          StubDecoderRoute::kUnsupported &&
                      !listed_in_any_route,
                  "expected fp8/bf8 batch instruction to keep exact unsupported-seed decode parity and selector exclusion")) {
        return 1;
      }
      for (const StubDecoderRouteManifest& manifest :
           GetStubDecoderRouteManifests()) {
        ...
        if (!Expect(MatchesUnsupportedSeedDecode(via_entrypoint, *seed) &&
                        MatchesUnknownHelperSurface(via_entrypoint),
                    "expected fp8/bf8 batch instruction to keep exact route-keyed unsupported parity")) {
          return 1;
        }
      }
      continue;
    ...
    if (!Expect(
            via_name.status == StubDecodeStatus::kDecodedStub &&
                ...
                route_manifest->route_name == route_info->route_name &&
                route_manifest->route_priority == route_info->route_priority &&
                route_contains_instruction(route_info->route,
                                           instruction_name) &&
                SelectStubDecoderRoute(instruction_name) == route_info->route,
            "expected fp8/bf8 batch instruction to preserve exact route-keyed parity, selector/manifest consistency, and local operand surfaces")) {
      return 1;
    }
    ...
    if (!Expect(GetStubOpcodeShapeName(via_name.opcode_shape) ==
                ...
                "expected fp8/bf8 batch instruction to keep exact helper-name parity and coverage")) {
      return 1;
    }
  }
  for (std::size_t i = 50; i < fp8_bf8_seeded_instructions.size(); ++i) {
    const std::string_view instruction_name = fp8_bf8_seeded_instructions[i];
    const DecoderSeedInfo* seed = FindDecoderSeedInfo(instruction_name);
    if (!Expect(seed != nullptr,
                "expected trailing fp8/bf8 batch instruction to remain present in the seed catalog")) {
      return 1;
    }
    ...
    if (route_info == nullptr) {
      ...
      if (!Expect(MatchesUnsupportedSeedDecode(decoded, *seed) &&
                      ...
                      SelectStubDecoderRoute(instruction_name) ==
                          StubDecoderRoute::kUnsupported &&
                      !listed_in_any_route,
                  "expected trailing fp8/bf8 batch instruction to keep exact unsupported-seed decode parity and selector exclusion")) {
        return 1;
      }
      for (const StubDecoderRouteManifest& manifest :
           GetStubDecoderRouteManifests()) {
        ...
        if (!Expect(MatchesUnsupportedSeedDecode(via_entrypoint, *seed) &&
                        MatchesUnknownHelperSurface(via_entrypoint),
                    "expected trailing fp8/bf8 batch instruction to keep exact route-keyed unsupported parity")) {
          return 1;
        }
      }
      continue;
    ...
    if (!Expect(
            via_name.status == StubDecodeStatus::kDecodedStub &&
                ...
                route_contains_instruction(route_info->route, instruction_name) &&
                SelectStubDecoderRoute(instruction_name) == route_info->route,
            "expected trailing fp8/bf8 batch instruction to preserve exact route-keyed parity, selector/manifest consistency, and local operand surfaces")) {
      return 1;
    }
  ...
  if (!Expect(vop3p_seeded_instructions.size() == 62,
              "expected VOP3P family to expose 62 seeded instructions")) {
    return 1;
  }
  for (std::size_t i = 0; i < 50; ++i) {
    const std::string_view instruction_name = vop3p_seeded_instructions[i];
    const DecoderSeedInfo* seed = FindDecoderSeedInfo(instruction_name);
    if (!Expect(seed != nullptr,
                "expected VOP3P batch instruction to remain present in the seed catalog")) {
      return 1;
    }
    ...
    const StubDecoderRouteInfo* route_info =
        FindStubDecoderRouteInfo(instruction_name);
    if (!Expect(route_info != nullptr &&
                    route_info->route == StubDecoderRoute::kVop3p &&
                    route_contains_instruction(route_info->route,
                                               instruction_name),
                "expected VOP3P batch instruction to keep exact route-keyed parity")) {
      return 1;
    }
    ...
    if (!Expect(
            via_name.status == StubDecodeStatus::kDecodedStub &&
                ...
                route_manifest->route_priority == route_info->route_priority &&
                SelectStubDecoderRoute(instruction_name) == route_info->route,
            "expected VOP3P batch instruction to preserve exact route-keyed parity, selector/manifest consistency, and local operand surfaces")) {
      return 1;
    }
  ...
  if (!Expect(Vop3pTailBatchRouteManifestCountParityMatchesSeedCatalog(),
              "expected VOP3P family to keep exact route-manifest count parity across the remaining tail batch")) {
    return 1;
  }
  ...
  for (std::size_t i = 50; i < vop3p_seeded_instructions.size(); ++i) {
    ...
    const DecoderSeedInfo* seed = FindDecoderSeedInfo(instruction_name);
    if (!Expect(seed != nullptr,
                "expected VOP3P tail batch instruction to remain present in the seed catalog")) {
      return 1;
    }
    ...
    if (!Expect(route_info != nullptr &&
                    ...
                    IsInstructionListedForRoute(route_info->route,
                                                instruction_name),
                "expected VOP3P tail batch instruction to keep exact route-keyed parity")) {
      return 1;
    }
    ...
    if (!Expect(
            via_name.status == StubDecodeStatus::kDecodedStub &&
                ...
                route_manifest->route_priority == route_info->route_priority &&
                SelectStubDecoderRoute(instruction_name) == route_info->route,
            "expected VOP3P tail batch instruction to preserve exact route-keyed parity, selector/manifest consistency, and local operand surfaces")) {
      return 1;
    }
    ...
    if (!Expect(GetStubOpcodeShapeName(via_name.opcode_shape) ==
                    ...
                    AllSlotKindHelperNamesKnown(via_name) &&
                    AllValueClassHelperNamesKnown(via_name),
                "expected VOP3P tail batch instruction to keep exact helper-name parity and coverage")) {
      return 1;
    }
  }
  ...
  for (const StubDecoderEntrypointManifest& manifest :
    ...
    struct FamilyMatchedSuffixedNearMissExpectation {
      std::string_view instruction_name;
      StubDecoderRoute route;
    ...
    for (const FamilyMatchedSuffixedNearMissExpectation& expectation :
         {FamilyMatchedSuffixedNearMissExpectation{
              "V_PK_ADD_BF16_X",
              StubDecoderRoute::kVop3p,
              "kVop3pPackedBinary",
              "kVectorAlu",
      ...
      const StubDecoderRouteInfo synthetic_family_matched_suffix{
          expectation.instruction_name,
          manifest.route,
          "kSyntheticRouteInfoFamilyMatchedSuffixNearMiss",
          manifest.route_priority + 230u,
          DecodeSeedHint::kUnknown,
          "SYNTHETIC_FAMILY_MATCHED_SUFFIX_NEAR_MISS_ENC",
          2300u + manifest.route_priority,
          23u + manifest.route_priority,
          false,
          false,
      };
      ...
      StubDecoderRouteInfo synthetic_family_matched_suffix_valid_wrong_hint =
          synthetic_family_matched_suffix;
      synthetic_family_matched_suffix_valid_wrong_hint.route_name =
          "kSyntheticRouteInfoFamilyMatchedSuffixNearMissValidWrongHint";
      synthetic_family_matched_suffix_valid_wrong_hint.route_priority =
          manifest.route_priority + 235u;
      synthetic_family_matched_suffix_valid_wrong_hint.decode_hint =
          AlternateDecodeHintForRoute(manifest.route);
      const StubDecodedInstruction
      ...
      StubDecoderRouteInfo synthetic_family_matched_suffix_invalid_hint =
          synthetic_family_matched_suffix;
      synthetic_family_matched_suffix_invalid_hint.route_name =
          "kSyntheticRouteInfoFamilyMatchedSuffixNearMissInvalidHint";
      synthetic_family_matched_suffix_invalid_hint.route_priority =
          manifest.route_priority + 245u;
      synthetic_family_matched_suffix_invalid_hint.decode_hint =
          static_cast<DecodeSeedHint>(99);
    ...
    struct FamilyMatchedSplitTokenNearMissExpectation {
      std::string_view instruction_name;
      StubDecoderRoute route;
      std::string_view opcode_shape_name;
      std::string_view execution_domain_name;
    ...
    for (const FamilyMatchedSplitTokenNearMissExpectation& expectation :
         {FamilyMatchedSplitTokenNearMissExpectation{
              "V_PK__ADD_BF16",
              StubDecoderRoute::kVop3p,
              "kVop3pPackedBinary",
              "kVectorAlu",
      ...
      const StubDecoderRouteInfo synthetic_family_matched_split_token{
          expectation.instruction_name,
          manifest.route,
          "kSyntheticRouteInfoFamilyMatchedSplitTokenNearMiss",
          manifest.route_priority + 280u,
          DecodeSeedHint::kUnknown,
          "SYNTHETIC_FAMILY_MATCHED_SPLIT_TOKEN_NEAR_MISS_ENC",
          2800u + manifest.route_priority,
          28u + manifest.route_priority,
          false,
          false,
      };
      const StubDecodedInstruction family_matched_split_token_decoded =
          DecodeStubInstruction(synthetic_family_matched_split_token);
      const bool matching_route = manifest.route == expectation.route;
      if (!Expect(
              family_matched_split_token_decoded.status ==
                      StubDecodeStatus::kDecodedStub &&
                  MatchesRouteInfoPayload(family_matched_split_token_decoded,
                                          synthetic_family_matched_split_token) &&
                  family_matched_split_token_decoded.entrypoint_name ==
                      manifest.entrypoint_name,
              "expected split-token family near-miss synthetic route-info to preserve caller metadata")) {
        return 1;
      }
      if (matching_route) {
        if (!Expect(
                std::string_view(GetStubOpcodeShapeName(
                    family_matched_split_token_decoded.opcode_shape)) ==
                        expectation.opcode_shape_name &&
                    std::string_view(GetStubExecutionDomainName(
                        family_matched_split_token_decoded.execution_domain)) ==
                        expectation.execution_domain_name &&
                    MatchesTopLevelFlags(family_matched_split_token_decoded,
                                         expectation.uses_accumulator,
                                         expectation.uses_tensor_memory,
                                         expectation.uses_scale_path,
                                         expectation.uses_paired_operands) &&
                    MatchesLayout(family_matched_split_token_decoded,
                                  expectation.matching_layout),
                "expected matching-route split-token family near-miss to preserve current family-level classification")) {
          return 1;
        }
      } else {
        if (!Expect(
                MatchesTopLevelFlags(family_matched_split_token_decoded,
                    ...
                    std::string_view(GetStubExecutionDomainName(
                        ...
                        "kUnknown" &&
                    MatchesLayout(family_matched_split_token_decoded,
                                  expectation.nonmatching_layout),
                "expected non-matching-route split-token family near-miss to keep exact top-level fallback behavior")) {
          return 1;
        }
      }
      if (expectation.instruction_name == "V_WMMA__F32_16X16X4_F32_w32") {
        if (!Expect(
                MatchesRoleBindingSequence(
                    family_matched_split_token_decoded,
                    {{StubOperandRole::kSource0, 1, false, false},
                     {StubOperandRole::kSource1, 1, false, false},
                     {StubOperandRole::kAccumulator, 1, false, false},
                     {StubOperandRole::kDestination, 1, true, false}}) &&
                    ...
                    MatchesDescriptorSequence(
                        ...
                        {{StubOperandRole::kDestination,
                         ...
                         {StubOperandRole::kAccumulator,
                          ...
                          1,
                          false}}),
                "expected split-token WMMA near-miss to preserve generic matrix operand surfaces")) {
          return 1;
        }
      } else if (expectation.instruction_name ==
                 "V_WMMA_SCALE__F32_16X16X128_F8F6F4") {
        if (!Expect(
                MatchesRoleBindingSequence(
                    family_matched_split_token_decoded,
                    ...
                    MatchesDescriptorSequence(
                        ...
                        {{StubOperandRole::kDestination,
                         ...
                         {StubOperandRole::kScale,
                          ...
                          1,
                          false}}),
                "expected split-token WMMA scale near-miss to preserve generic scale operand surfaces")) {
          return 1;
        }
      } else if (!expectation.preserve_generic_operands_on_nonmatching_route) {
        if (!Expect(
                family_matched_split_token_decoded.operand_roles.binding_count ==
                        0 &&
                    family_matched_split_token_decoded.operand_slots
                            .binding_count == 0 &&
                    family_matched_split_token_decoded.operand_descriptors
                            .descriptor_count == 0,
