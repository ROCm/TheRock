...
bool Fp8Bf8LeadingBatchRouteManifestCountParityMatchesSeedCatalog() {
  const auto seeded_instructions =
...
bool Fp8Bf8TailBatchRouteManifestCountParityMatchesSeedCatalog() {
  const auto seeded_instructions =
      GetSeededInstructionNames(SeedFamily::kFp8Bf8);
  if (seeded_instructions.size() != 87 ||
      seeded_instructions[49] != "V_CVT_SCALE_PK8_F16_FP8" ||
...
bool Vop3pLeadingBatchRouteManifestCountParityMatchesSeedCatalog() {
  ...
  if (manifest == nullptr ||
      ...
      manifest->instruction_count !=
          CountSeededInstructionsForRoute(StubDecoderRoute::kVop3p)) {
    return false;
  }
...
bool Vop3pTailBatchRouteManifestCountParityMatchesSeedCatalog() {
  ...
  if (manifest == nullptr ||
      ...
      manifest->instruction_count !=
          GetStubDecoderRouteInstructions(StubDecoderRoute::kVop3p).size() ||
      manifest->xml_backed_count !=
          CountXmlBackedSeededInstructionsForRoute(StubDecoderRoute::kVop3p) ||
      manifest->llvm_only_count !=
          CountLlvmOnlySeededInstructionsForRoute(StubDecoderRoute::kVop3p) ||
      manifest->target_specific_count !=
...
int main() {
  ...
  if (!Expect(Fp8Bf8TailBatchRouteManifestCountParityMatchesSeedCatalog(),
              "expected fp8/bf8 family to keep exact route-manifest count parity across the remaining tail batch")) {
    return 1;
  }
