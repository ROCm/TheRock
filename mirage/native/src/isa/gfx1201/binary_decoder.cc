...
OperandDescriptor MakeVectorRegisterDescriptor(OperandRole role,
  ...
  descriptor.access = access;
  descriptor.fragment_shape =
      MakeVectorFragmentShape(component_count, element_bit_width);
  descriptor.component_count = component_count;
  return descriptor;
}
...
OperandDescriptor MakePackedVectorDescriptor(OperandRole role,
                                             OperandSlotKind slot_kind,
                                             OperandAccess access,
                                             std::uint8_t element_bit_width = 16) {
  OperandDescriptor descriptor;
  descriptor.role = role;
  descriptor.slot_kind = slot_kind;
  descriptor.value_class = OperandValueClass::kPackedVector;
  descriptor.access = access;
  descriptor.fragment_shape = MakePackedFragmentShape(2u, element_bit_width);
...
InstructionOperand DescribeSourceOperand(InstructionOperand operand,
  ...
  if (operand.kind == OperandKind::kSgpr) {
    return operand.WithDescriptor(MakeScalarRegisterDescriptor(
        role, slot_kind, OperandAccess::kRead, element_bit_width));
  }
  ...
  return operand.WithDescriptor(
      MakeImmediateDescriptor(role, slot_kind, element_bit_width));
}
...
InstructionOperand DescribeVectorPairDestinationOperand(InstructionOperand operand) {
  return operand.WithDescriptor(MakeVectorRegisterDescriptor(
      OperandRole::kDestination, OperandSlotKind::kDestination,
  ...
  *words_consumed = 0;
  ...
  if (instruction_name == "S_ENDPGM") {
    ...
    *words_consumed = 2;
             ...
             instruction_name == "DS_PK_ADD_RTN_F16" ||
             instruction_name == "DS_PK_ADD_F16" ||
             instruction_name == "DS_PK_ADD_RTN_BF16" ||
             instruction_name == "DS_PK_ADD_BF16" ||
             instruction_name == "DS_MIN_NUM_RTN_F32" ||
             instruction_name == "DS_MIN_NUM_F32" ||
             instruction_name == "DS_MAX_NUM_RTN_F32" ||
             instruction_name == "DS_MAX_NUM_F32" ||
    ...
    if (has_second_data) {
      ...
      if (!DecodeVectorRegisterSource(ExtractBits(words[1], 16, 8), &vdata1,
                                      error_message)) {
        return false;
      }
      described_vdata1 =
        ...
        *instruction = DecodedInstruction::SixOperand(
            ...
            InstructionOperand::Imm32(ExtractBits(word, 0, 8))
                .WithDescriptor(MakeImmediateDescriptor(
                    OperandRole::kUnknown, OperandSlotKind::kUnknown, 8u)),
            InstructionOperand::Imm32(ExtractBits(word, 8, 8))
                .WithDescriptor(MakeImmediateDescriptor(
                    OperandRole::kUnknown, OperandSlotKind::kUnknown, 8u)));
      }
    ...
    *words_consumed = 2;
    ...
    InstructionOperand described_vdst = DescribeVectorDestinationOperand(vdst);
    if (instruction_name == "DS_LOAD_B64") {
      described_vdst = DescribeWideVectorDestinationOperand(vdst);
    } else if (instruction_name == "DS_LOAD_B96") {
    ...
    *instruction = DecodedInstruction::ThreeOperand(
        instruction_name, described_vdst,
        DescribeSourceOperand(vaddr, OperandRole::kSource0,
                              OperandSlotKind::kSource0),
        InstructionOperand::Imm32(ExtractBits(word, 0, 8))
            .WithDescriptor(MakeImmediateDescriptor(OperandRole::kSource1,
    ...
    *instruction = DecodedInstruction::Nullary(instruction_name);
    *words_consumed = 2;
  } else if (instruction_name == "GLOBAL_LOAD_ADDTID_B32") {
    if (words.size() < 2u) {
      if (error_message != nullptr) {
        *error_message = std::string(instruction_name) + " requires 2 dwords";
      }
    ...
    if (raw_saddr >= WaveExecutionState::kScalarRegisterCount) {
      if (error_message != nullptr) {
        *error_message = "GLOBAL ADDTID load scalar address out of range";
      }
      return false;
    }
    ...
    *instruction = DecodedInstruction::ThreeOperand(
        ...
        DescribeWideSourceOperand(saddr, OperandRole::kSource0,
                                  OperandSlotKind::kSource0),
    ...
    *words_consumed = 2;
             ...
             instruction_name == "GLOBAL_LOAD_D16_HI_U8" ||
             instruction_name == "GLOBAL_LOAD_D16_HI_I8" ||
             instruction_name == "GLOBAL_LOAD_D16_HI_B16") {
    if (words.size() < 2u) {
      if (error_message != nullptr) {
        *error_message = std::string(instruction_name) + " requires 2 dwords";
      }
    ...
    if (instruction_name == "GLOBAL_LOAD_B64" ||
      ...
      described_dst = DescribeWideVectorDestinationOperand(dst, 4u);
               ...
               instruction_name == "GLOBAL_LOAD_D16_HI_I8" ||
               instruction_name == "GLOBAL_LOAD_D16_HI_B16") {
      described_dst = DescribeReadWriteVectorOperand(
          dst, OperandRole::kDestination, OperandSlotKind::kDestination, 16u);
    }
    ...
    *instruction = DecodedInstruction::FourOperand(
        ...
        InstructionOperand::Imm32(static_cast<std::uint32_t>(
            ...
            .WithDescriptor(MakeImmediateDescriptor(OperandRole::kSource2,
                                                   ...
                                                   13u)));
    *words_consumed = 2;
  } else if (instruction_name == "GLOBAL_STORE_ADDTID_B32") {
    if (words.size() < 2u) {
      if (error_message != nullptr) {
        *error_message = std::string(instruction_name) + " requires 2 dwords";
      }
    ...
    if (raw_saddr >= WaveExecutionState::kScalarRegisterCount) {
      if (error_message != nullptr) {
        *error_message = "GLOBAL ADDTID store scalar address out of range";
      }
      return false;
    }
    const InstructionOperand saddr =
    ...
    *instruction = DecodedInstruction::ThreeOperand(
        ...
        DescribeSourceOperand(vdata, OperandRole::kSource0,
                              OperandSlotKind::kSource0),
        DescribeWideSourceOperand(saddr, OperandRole::kSource1,
                                  OperandSlotKind::kSource1),
        InstructionOperand::Imm32(0u).WithDescriptor(
            MakeImmediateDescriptor(OperandRole::kUnknown,
                                    OperandSlotKind::kUnknown)));
    ...
    *words_consumed = 2;
             ...
             instruction_name == "GLOBAL_STORE_D16_HI_B8" ||
             instruction_name == "GLOBAL_STORE_D16_HI_B16") {
    if (words.size() < 2u) {
      if (error_message != nullptr) {
        *error_message = std::string(instruction_name) + " requires 2 dwords";
      }
    ...
    if (instruction_name == "GLOBAL_STORE_B64") {
      ...
      described_vdata = DescribeWideVectorSourceOperand(
          vdata, OperandRole::kSource0, OperandSlotKind::kSource0, 4u);
    } else if (instruction_name == "GLOBAL_STORE_D16_HI_B8") {
      described_vdata = DescribePackedSourceOperand(
          vdata, OperandRole::kSource0, OperandSlotKind::kSource0, 8u);
    } else if (instruction_name == "GLOBAL_STORE_D16_HI_B16") {
      described_vdata = DescribePackedSourceOperand(
          vdata, OperandRole::kSource0, OperandSlotKind::kSource0, 16u);
    }
    ...
    *words_consumed = 2;
             ...
             instruction_name == "GLOBAL_ATOMIC_PK_ADD_F16" ||
             instruction_name == "GLOBAL_ATOMIC_PK_ADD_BF16" ||
    ...
    *words_consumed = 2;
             ...
             instruction_name == "GLOBAL_ATOMIC_ORDERED_ADD_B64" ||
    ...
    *words_consumed = 1 + literal_words_consumed;
             ...
             instruction_name == "V_CVT_F32_BF8" ||
    ...
    *words_consumed = 1 + literal_words_consumed;
             ...
             instruction_name == "V_CVT_PK_F32_FP8" ||
             instruction_name == "V_CVT_PK_F32_BF8") {
    InstructionOperand dst;
    if (!DecodeVectorDestination(ExtractBits(word, 17, 8), &dst, error_message)) {
      return false;
    }
    ...
    if (instruction_name == "V_CVT_PK_F32_FP8" ||
        instruction_name == "V_CVT_PK_F32_BF8") {
      *instruction = DecodedInstruction::Unary(
          instruction_name, DescribeVectorPairDestinationOperand(dst),
          DescribePackedSourceOperand(src0, OperandRole::kSource0,
                                      OperandSlotKind::kSource0, 8u));
    } else {
      *instruction = DecodedInstruction::Unary(
          instruction_name, DescribeWideVectorDestinationOperand(dst),
          DescribeSourceOperand(src0, OperandRole::kSource0,
