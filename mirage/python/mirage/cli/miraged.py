"""Command-line entrypoint for miraged."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="miraged")
    parser.add_argument(
        "command",
        nargs="?",
        default="boot",
    ...
    parser.add_argument(
        "--state-path",
        help="Optional lifecycle snapshot path for boot/status inspection",
    )
    parser.add_argument(
        "--foreground",
        action="store_true",
        help="Accepted for forward compatibility; boots always run in-process today",
    )
    parser.add_argument(
        "--format",
        choices=("text", "json"),
