 ...
 * Same DDI table as MCDM but targeting WDDM 2.6 display driver.
 */
    ...
    )
{
    ...
    DriverInitData.Version = DXGKDDI_INTERFACE_VERSION_WDDM1_3;
    ...
    KdPrint(("AmdGpuWddm: DriverEntry calling DxgkInitialize, WDDM version=0x%X\n",
        DriverInitData.Version));

    /* Write DriverEntry marker to registry */
    {
        ...
        if (NT_SUCCESS(s)) {
            RtlInitUnicodeString(&ValName, L"DriverEntry");
            Val = 1;
    ...
    {
        NTSTATUS status = DxgkInitialize(DriverObject, RegistryPath, &DriverInitData);
        KdPrint(("AmdGpuWddm: DxgkInitialize returned 0x%08X\n", status));

        {
            ...
            if (NT_SUCCESS(s)) {
                RtlInitUnicodeString(&ValName, L"DxgkInitResult");
                ZwSetValueKey(hKey, &ValName, 0, REG_DWORD,
