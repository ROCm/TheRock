...
from .archive_util import open_archive_for_write
from .artifacts import ArtifactPopulator
from . import artifact_builder
from .hash_util import calculate_hash, write_hash
...
def create_artifact_archive(
    *,
    ...
    with open_archive_for_write(
        output_path, compression_type, compression_level
    ) as arc:
    ...
    if hash_file:
        digest = calculate_hash(output_path, hash_algorithm)
        write_hash(hash_file, digest)
