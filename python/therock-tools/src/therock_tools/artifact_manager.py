...
Usage:
    ...
    therock-artifact-manager fetch \
...
from .cmake_amdgpu_targets import (
    amdgpu_family_map,
    expand_families,
)
...
from .artifacts import ArtifactName, ArtifactPopulator
from .fileset_tool import create_artifact_archive
...
def get_topology(topology_path: Path) -> BuildTopology:
    """Load a BUILD_TOPOLOGY.toml file from an explicit caller-provided path."""
...
def compress_artifact(request: CompressRequest) -> Optional[Path]:
    """Compress a single artifact directory as a TheRock artifact archive."""
    try:
        ...
        create_artifact_archive(
            artifact_paths=[request.source_dir],
            output_path=request.archive_path,
            compression_type=request.compression_type,
            compression_level=request.compression_level,
            hash_file=Path(str(request.archive_path) + ".sha256sum"),
        )
...
def _add_common_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to BUILD_TOPOLOGY.toml",
    )
