...
1. Visit the [nightly multi-arch tarball feed](https://rocm.nightlies.amd.com/tarball-multi-arch/)
...
1. Visit the [dev multi-arch tarball feed](https://rocm.devreleases.amd.com/tarball-multi-arch/)
1. Look for files matching your GPU family. Files are named: `therock-dist-linux-{GPU_FAMILY}-{VERSION}.tar.gz`
...
> [!TIP]
> Browse the public multi-arch tarball feeds to find available versions and GPU families. The version string to use with `--release` is always the portion of the filename between the GPU family and `.tar.gz`.
