...
2. It calls `StageImpactAnalyzer.required_stages_for_component()` to walk
   the build graph *upstream* from the owning stage, collecting every stage
   whose artifacts the owning stage depends on.
3. The resulting `build_stages` list is embedded in `external_repo_config`
...
2. **Wire up the CMake variable** in the component's TheRock integration
   (e.g. `cmake/therock_my_component.cmake`) to consume
   `THEROCK_MY_COMPONENT_SOURCE_DIR` when set.

3. **Create the caller workflow** in the external repo (see the example
