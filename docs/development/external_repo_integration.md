...
| --------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `repository`          | Yes      | GitHub repo in `owner/name` format (e.g. `"ROCm/ROCgdb"`). Must match a registered repo name in `REPO_CONFIGS` (case-insensitive).                                                                                      |
| `ref`                 | Yes      | Git SHA, branch, or tag to check out from `repository`.                                                                                                                                                                 |
| `extra_cmake_options` | No       | CMake flags forwarded to the configure step of every stage (e.g. `-DTHEROCK_USE_EXTERNAL_ROCGDB=ON`). Passed unconditionally when `external_repo_config` is set; CMake silently ignores flags it does not consume.      |
| `skip_packaging`      | No       | `true` to suppress DEB/RPM packages, Python wheels, and PyTorch wheel jobs. Defaults to `false`.                                                                                                                        |
| `family_overrides`    | No       | JSON object mapping GPU family names to per-platform config overrides (e.g. test runners, test labels). Merged into the family matrix at CI configuration time. Useful for adding runners not in TheRock's default set. |
| `projects`            | No       | Reserved field embedded in `external_repo_config` for future use. Not currently consumed by any workflow.                                                                                                               |
