...
- `--preset linux-release-asan-compact`: Same as `linux-release-asan` plus size-oriented flags (`THEROCK_ASAN_COMPACT`): `-fsanitize-address-outline-instrumentation`, `-Oz`, `-gz`, `-gline-tables-only`, `-fdata-sections`/`-ffunction-sections`, `-flto`, and `-Wl,--gc-sections`. `--offload-compress` is added only for HIP (`CMAKE_HIP_FLAGS` / `COMPILE_LANGUAGE:HIP`), not host C/C++. These are injected only into Clang/HIP subprojects. **Requires a fresh build directory:** compact flags use `*_FLAGS_INIT` (applied on first configure only) and `FORCE` `-Oz` on Release. Do not enable or disable `THEROCK_ASAN_COMPACT` in an existing tree; delete the build directory and reconfigure with this preset (or `-DTHEROCK_ASAN_COMPACT=ON`) from the start.
