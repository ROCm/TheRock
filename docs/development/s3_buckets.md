...
- `--bucket-config-file <path>`, on `artifact_manager.py` subcommands. Prefer
  this: it makes a single invocation self-describing, and `artifact_manager copy`
  builds a source and a destination backend in one process, which no
  process-wide setting can express.
...
everything under, and is folded into every key the tools generate;
`cdn_rules` map a key prefix to a public URL prefix, longest match first.
