...
To enable the **content cache** (recommended for repeated model loads), start
the worker with `HIP_REMOTE_CACHE=1`:

```bash
HIP_REMOTE_CACHE=1 nohup ./hip-worker > /tmp/hip-worker.log 2>&1 &
```

When caching is enabled the worker runs in sequential mode (one client at a
time) so the GPU cache persists across client sessions. The cache uses 33% of
VRAM by default; override with `HIP_REMOTE_CACHE_SIZE_MB`.
...
| `HIP_REMOTE_PROFILE`   | Set to `1` to print API call counts at exit     |
| `HIP_REMOTE_CACHE`     | Set to `1` on both client and worker to enable content cache |
...
- **Content-addressed weight cache**: When `HIP_REMOTE_CACHE=1` is set on
  both client and worker, H2D memory transfers are hashed (XXH3) and cached
  in GPU memory on the worker. On repeated model loads the client sends only
  the hash and the worker does a fast D2D copy, skipping the network transfer.
  SDXL (7 GB weights) loads in ~5s on warm cache vs ~300s uncached.
