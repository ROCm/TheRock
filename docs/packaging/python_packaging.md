...
wheel a PEP 440 version carrying the `asan` build variant, and packages the
shared Clang ASAN runtime in `rocm-sdk-core`. The release channel remains
independent: examples are `10.2.0.dev0+<sha>.asan` for dev,
`10.2.0a20260807+asan` for nightly, and `10.2.0+asan` for a release build.
...
When no explicit version is supplied, local packaging uses `<base>+asan`.
CI passes the channel-specific version generated by
`compute_rocm_package_version.py --build-variant=asan`. An explicit
`--version` must use the artifact's ROCm base version and contain an `asan`
...
```text
rocm-10.2.0+asan.tar.gz
rocm_sdk_core-10.2.0+asan-*.whl
rocm_sdk_libraries-10.2.0+asan-*.whl
rocm_sdk_device_gfx942-10.2.0+asan-*.whl
rocm_sdk_devel-10.2.0+asan-*.whl
```

Release workflows publish these wheels to the isolated `whl-next-asan` index.
After installation, `python -m rocm_sdk test` checks that ASAN-linked ELFs
resolve the packaged runtime through a relative RPATH. Local packaging consumes
TheRock's structured `artifacts/` directory, not an already-flattened
`therock-dist-*.tar.gz` SDK archive.
