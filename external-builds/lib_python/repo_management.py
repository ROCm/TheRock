...
class RockProjectRepo():
    def __init__(self,
        ...
        ROCK_BUILDER_ROOT_DIR = Path(os.environ["ROCK_BUILDER_ROOT_DIR"])
        self.project_patch_dir_base = ROCK_BUILDER_ROOT_DIR / "patches" / project_name
    ...
    def __get_latest_file(self, path, extension=None):
        ...
        if path is not None:
            ...
            if not ((search_key.endswith('\\')) or (search_key.endswith('/')) or (search_key.endswith('*'))):
    ...
    def do_build(self, build_cmd):
        return self.__exec_subprocess_cmd(build_cmd)
