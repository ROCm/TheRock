    ...
    if args.repo_hashtag is None:
        ...
        if pin_major == 2:
            if pin_minor == 7:
                ...
                raise ValueError(
                    f"Unsupported torch minor version: {pin_minor}, torch version: {pin_major}.{pin_minor} (did you check out torch first)"
                )
        else:
            raise ValueError(
                f"Unsupported torch major version: {pin_major}, torch version: {pin_major}.{pin_minor} (did you check out torch first)"
            )
