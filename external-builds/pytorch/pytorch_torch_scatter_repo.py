...
Primary usage:
    ...
    ./pytorch_torch_scatter_repo.py checkout

The checkout process combines the following activities:

* Clones the repository into `THIS_MAIN_REPO_NAME` with a requested `--repo-hashtag`
  tag (default to latest release).
* Configures submodules to be ignored for any local changes.
* Optionally runs `hipify` to prepare sources for AMD GPU and commits the result to the
...
THIS_MAIN_REPO_NAME = "torch_scatter"
THIS_DIR = Path(__file__).resolve().parent

DEFAULT_ORIGIN = "https://github.com/rusty1s/pytorch_scatter.git"
...
def main(cl_args: list[str]):
    ...
    p = argparse.ArgumentParser("pytorch_torch_scatter_repo.py")
    sub_p = p.add_subparsers(required=True)
    checkout_p = sub_p.add_parser(
        "checkout", help="Clone PyTorch Scatter locally and checkout"
    )
    ...
    if args.require_related_commit and not has_related_commit:
        raise ValueError(
            f"Could not find torch_scatter in '{args.torch_dir}/related_commits' (did you mean to set a different --torch-dir?)"
        )
