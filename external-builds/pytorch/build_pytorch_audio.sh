...
# Usage:
#   build_pytorch_audio.sh  (uses default source dir of src/audio/)
#   build_pytorch_audio.sh path/to/audio_source_dir
...
SOURCE_DIR="${1:-$SCRIPT_DIR/src/audio}"
...
if [ ${DO_INSTALL_STEP} -eq 1 ]; then
	...
	if [ ! -z "$latest_wheel_file" ]; then
		...
		echo "Failed to build and find pytorch audio wheel for pip install in directory $SOURCE_DIR/dist"
		exit 1
	fi
fi
cd $SCRIPT_DIR
