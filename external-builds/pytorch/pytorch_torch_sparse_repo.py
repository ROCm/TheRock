...
Primary usage:
    ...
    ./pytorch_torch_sparse_repo.py checkout
...
def main(cl_args: list[str]):
    ...
    p = argparse.ArgumentParser("pytorch_torch_sparse_repo.py")
    sub_p = p.add_subparsers(required=True)
    checkout_p = sub_p.add_parser(
        "checkout", help="Clone PyTorch Sparse locally and checkout"
    )
    ...
    if args.require_related_commit and not has_related_commit:
        raise ValueError(
            f"Could not find torch_sparse in '{args.torch_dir}/related_commits' (did you mean to set a different --torch-dir?)"
        )
