...
* To install the latest nightly rocm packages for gfx110-all into the venv:

    ```
    python setup_venv.py .venv --packages rocm[libraries,devel] \
        --index-name nightly --index-subdir gfx110-all
    ```
    ...
    python -m pip install rocm[libraries,devel] --index-url=https://.../gfx110-all
    deactivate
    ```
"""
...
def main(argv: list[str]):
    ...
    if not all_subdir_sets_congruent and subdirs:
        ...
        index_subdir_help += ", such as 'gfx110-all'"
    else:
        index_subdir_help += "."
