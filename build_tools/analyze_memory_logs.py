...
class MemoryLogAnalyzer:
    ...
    def analyze_phase(self, phase: str, samples: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def generate_report(self, detailed: bool = False) -> str:
        ...
        for analysis in phase_analyses:
            ...
            lines.append(f"{prefix} {phase_name:<37} {peak:<12} {avg:<12} {severity:<10}")
        ...
        if detailed:
            ...
            for analysis in phase_analyses:
                ...
                lines.append(f"    Peak: {analysis['max_memory_percent']:.1f}% ({analysis['peak_memory_gb']:.2f} GB)")
                lines.append(f"    Range: {analysis['min_memory_percent']:.1f}% - {analysis['max_memory_percent']:.1f}%")
                ...
                if analysis['severity'] in ['CRITICAL', 'HIGH']:
                    lines.append(f"  [!] WARNING: This phase had {analysis['severity']} memory usage!")
        ...
        critical_phases = [a for a in phase_analyses if a['severity'] in ['CRITICAL', 'HIGH']]
        if critical_phases:
            lines.append("")
            lines.append("[!] PHASES WITH HIGH MEMORY USAGE:")
            for analysis in critical_phases:
                lines.append(f"  - {analysis['phase']}: {analysis['max_memory_percent']:.1f}% peak")
                lines.append(f"    Consider reducing parallel jobs or increasing available memory")
        ...
        if high_swap:
            ...
            for analysis in high_swap:
                lines.append(f"  - {analysis['phase']}: {analysis['max_swap_percent']:.1f}% peak")
                lines.append(f"    Swap usage indicates memory pressure and will slow down builds")
            ...
            with open(os.environ["GITHUB_STEP_SUMMARY"], 'a') as f:
                ...
                if critical:
                    ...
                    for analysis in critical:
                        f.write(f"- **{analysis['phase']}**: {analysis['max_memory_percent']:.1f}% peak memory\n")
