...
class BuildTopology:
    ...
    def load_subproject_manifest(
        ...
        """Load artifact→subprojects manifest generated by CMake.
        ...
        """
        manifest_path = build_dir / "artifact_subprojects.json"
    ...
    def get_alias_to_artifact_map(
        ...
        Args:
            build_dir: Optional path to CMake build directory. If provided and
                      artifact_subprojects.json exists, uses CMake-generated
                      mappings for accurate subproject resolution.
        ...
        # Try to load CMake-generated manifest for accurate mappings
        manifest = None
        if build_dir:
            manifest = self.load_subproject_manifest(build_dir)

        for artifact in self.artifacts.values():
            # Artifact name always maps to itself
            alias_map[artifact.name.lower()] = artifact.name

            # Get subprojects from manifest (requires --build-dir with configured build)
