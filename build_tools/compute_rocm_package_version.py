...
Sample usage:
  ...
  python compute_rocm_package_version.py --release-type=nightly --build-variant=asan
  # rocm_package_version=10.1.0a20260807+asan
"""
...
def compute_version(
    ...
    Args:
        ...
        build_variant: Orthogonal build variant. ``asan`` adds an ASAN
            local-version marker to wheel versions.
    ...
    if build_variant not in ("release", "asan"):
    ...
    if package_type == "wheel":
        ...
        if build_variant == "asan":
            separator = "." if "+" in rocm_package_version else "+"
            rocm_package_version += f"{separator}asan"
...
def main(argv):
    ...
    release_type_group.add_argument(
        ...
        help=(
            "The type of package version to produce. " "'ci' uses dev-style versions."
        ),
    )
    release_type_group.add_argument(
    ...
    parser.add_argument(
        ...
        default="release",
        help="Orthogonal build variant. 'asan' adds an ASAN local-version marker.",
    )
