...
def test_monitoring_loop():
    ...
    try:
        ...
        assert len(monitor.samples) >= 3, f"Expected at least 3 samples, got {len(monitor.samples)}"
...
def test_command_execution():
    ...
    wrapper_script = Path(__file__).parent.parent / "github_actions" / "memory_wrapped_build.py"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "Memory monitoring started" in result.stdout or "Executing command" in result.stdout
