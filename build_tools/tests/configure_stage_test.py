...
class StageSkipTest(unittest.TestCase):
    ...
    def test_rocgdb_needs_debug_tools_and_compiler_runtime(self):
        """rocgdb lives in debug-tools which depends on compiler-runtime."""
        required = self.topology.get_stages_for_projects(["rocgdb"])
        self.assertIn("compiler-runtime", required)
        self.assertIn("debug-tools", required)
        self.assertNotIn("math-libs", required)
        self.assertNotIn("comm-libs", required)
        self.assertNotIn("media-libs", required)
