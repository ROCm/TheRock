...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to True when --run-id is set, False otherwise."
        ),
    )
    parser.add_argument(
        "--artifacts-cache-dir",
...
def resolve_package_inputs(args: argparse.Namespace) -> PackageInputs:
    ...
    Fetches artifacts when --run-id is set and fetches legacy DLLs from DVC when
    applicable, then enumerates the concrete files to install.
    """
    ...
    # --fetch-legacy-dlls defaults to True in --run-id mode, False for a local
    # build. Skip entirely when the package declares no legacy DLLs.
    fetch_legacy = args.fetch_legacy_dlls
    if fetch_legacy is None:
        fetch_legacy = bool(args.run_id)
