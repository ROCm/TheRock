...
are never read from the same directory. Aggregator install checks call
...
WHAT IS VALIDATED
  ...
  3. JAX wheels (jax_rocm7_* / jax_rocm10_* plugin and pjrt) promote correctly: the RC
...
class InputSet:
    ...
    def _non_pinned_triton(self) -> set[str]:
        ...
        matched = {
            ...
            if any(_triton_wheel_matches_pin(n, pin) for pin in pins)
        }
        if not matched:
            raise RuntimeError(
