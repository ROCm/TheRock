...
Stage 1 (pre-upload) coverage
-----------------------------
* ``parse_dependency_names`` — DEB/RPM control-field parsing
* ``iter_variant_configs`` / ``expected_dependencies`` — package.json rules (#6093)
* ``_compare_dep_lists`` — expected vs actual diff logic
* ``RunSummary`` / JSON report serialization
* ``_guess_base_package_name`` — transitive walk name mapping (optional mode)

Run (Python 3.10+)::

    python3 -m unittest build_tools.packaging.linux.tests.pkg_dependency_checker_test -v
"""
...
from packaging_utils import GFX_META, PackageConfig, read_package_json_file  # noqa: E402
...
# Epilog
# -----
# These tests validate Stage 1 tooling: dependency parsing, variant enumeration,
# package.json expectation logic, and report aggregation. Integration with real
# ``.deb`` files is exercised separately via Docker (see DEPENDENCY-CHECK/test-logs/).
