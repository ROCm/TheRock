...
class MainValidationTest(unittest.TestCase):
    ...
    def test_uninstall_requires_repo_url(self):
        with patch(
            "sys.argv",
            [
                "prog",
...
class ArgvFromCiEnvTest(unittest.TestCase):
    ...
    def test_builds_argv_for_uninstall_test_type(self):
        env = {
            "TEST_TYPE": "uninstall",
            "OS_PROFILE": "rhel10",
            "REPO_URL": "https://example.com/rpm/x86_64/",
            "GFX_ARCH": "gfx94X-dcgpu",
            "RELEASE_TYPE": "dev",
            "INSTALL_PREFIX": "/opt/rocm/core",
            "GPG_KEY_URL": "https://example.com/gpg",
        }
        with patch.dict(os.environ, env, clear=False):
            argv = native_linux_package_install_test._argv_from_ci_env()
        self.assertIsNotNone(argv)
        self.assertEqual(argv[argv.index("--test-type") + 1], "uninstall")
        self.assertIn("--gpg-key-url", argv)


class RunTestsTestTypeTest(unittest.TestCase):
    """Tests for run_tests() early exit paths for install and uninstall."""
    ...
    def test_install_skips_basic_verification(
        self, mock_basic, mock_repo_setup
    ):
    ...
    def test_uninstall_skips_basic_verification(
        self, mock_basic, mock_repo_setup, mock_uninstall
    ):
        args = self._base_args("uninstall")
        with _suppress_script_output():
            rc = native_linux_package_install_test.run_tests(args)
        self.assertEqual(rc, 0)
...
class UninstallPackagesTest(unittest.TestCase):
    """Tests for uninstall_deb_packages, uninstall_rpm_packages, run_uninstall_packages."""

    @patch("native_linux_package_install_test._run_streaming")
    def test_uninstall_deb_reverse_order_and_apt_remove(self, mock_streaming):
        ...
        self.assertTrue(t.uninstall_deb_packages())
        cmd = mock_streaming.call_args[0][0]
        self.assertEqual(cmd[0], "apt")
        self.assertEqual(cmd[1], "remove")
        self.assertEqual(cmd[2:], ["-y", "amdrocm-core-sdk", "amdrocm"])

    @patch("native_linux_package_install_test._run_streaming")
    def test_uninstall_rpm_uses_dnf_for_rhel(self, mock_streaming):
        mock_streaming.return_value = 0
        t = native_linux_package_install_test.NativeLinuxPackageInstallTest(
            repo_url="https://example.com",
            os_profile="rhel10",
        ...
        self.assertTrue(t.uninstall_rpm_packages())
        cmd = mock_streaming.call_args[0][0]
    ...
    def test_run_uninstall_packages_routes_to_deb(self, mock_uninstall):
        t = native_linux_package_install_test.NativeLinuxPackageInstallTest(
            repo_url="https://example.com",
            os_profile="ubuntu2404",
        )
        self.assertTrue(t.run_uninstall_packages())
        mock_uninstall.assert_called_once()
