...
* **package-file** (default) — ``Depends`` / ``Requires`` read from local
  ``.deb`` / ``.rpm`` files after ``build_package.py`` (pre-upload gate), or
* **installed** — packages on the host (``dpkg-query`` / ``rpm -q``) after
  repo install (post-install validation).

Primary pre-upload example (offline ``.deb`` / ``.rpm`` control fields)::
...
Post-install example (installed DEB, kpack nightly)::
...
Stage 1 scope (pre-upload milestone)
------------------------------------
* Compare ``package.json`` expected deps with deps embedded in built ``.deb``/``.rpm``
  files (``--mode package-file``, default).
* Enumerate kpack variants (GFX_META, per-GPU metas) without installing packages.
* Emit JSON/text reports for CI gating (Step 2 of pre-upload sequence).
* Does **not** install packages or walk transitive closure unless ``--transitive``.
"""
...
class VariantCheck:
    ...
    Fields
    ------
    label : Human-readable variant label (e.g. ``versioned meta (GFX_META)``).
    installed_name : On-disk or installed package name for this variant.
    package_found : Whether the variant exists (installed or in ``packages-dir``).
    expected_deps : Dependency names from ``package.json`` / ``process_main_dependencies``.
    actual_deps : Dependency names read from dpkg/rpm or package file control fields.
    missing : Expected deps absent from ``actual_deps``.
    not_installed : Declared deps not present on system (``--verify-installed`` only).
    extra_deps : Actual deps not expected by ``package.json`` rules (external policy).
    passed : True when ``missing`` and ``not_installed`` are both empty.
    """
...
def _suppress_packaging_noise(quiet: bool) -> None:
    """Disable verbose ``packaging_utils`` debug prints when ``--quiet`` is set."""
    if not quiet:
        return
...
def build_checker_config(args: argparse.Namespace) -> PackageConfig:
    """Build ``PackageConfig`` using the same mapping as ``build_package``."""
    from build_package import create_package_config

    return create_package_config(args)


def _dependency_field_key(pkg_type: str) -> str:
    """Return ``package.json`` dependency field name for DEB or RPM."""
...
def _run_capture(cmd: list[str]) -> subprocess.CompletedProcess[str]:
    """Run ``cmd`` and capture stdout/stderr without raising on non-zero exit."""
...
def read_package_file_dependencies(package_path: Path, pkg_type: str) -> list[str]:
    ...
    if pkg_type == "deb":
        ...
        result = _run_capture(["dpkg-deb", "-f", str(package_path), "Depends"])
        if result.returncode != 0:
            raise RuntimeError(
...
def find_package_files(packages_dir: Path, pkg_type: str) -> dict[str, Path]:
    """Index ``.deb`` / ``.rpm`` files by package name prefix (stem before version/arch)."""
    pkg_type = pkg_type.lower()
    index: dict[str, Path] = {}
    if pkg_type == "deb":
...
def _compare_dep_lists(
    ...
    *,
    ...
    """Diff expected vs actual dependency name lists.

    Returns ``(missing, not_installed, extra)`` where *missing* are expected
    names not in ``actual``, *extra* are in ``actual`` but not expected, and
    *not_installed* are declared deps absent from the system (installed mode).
    """
...
def check_package(
    ...
    *,
    ...
    """Verify all variants of one ``package.json`` entry against expectations.

    For each variant from ``iter_variant_configs``, reads actual dependencies
    from installed packages or local archives (per ``mode``), compares to
    ``expected_dependencies``, and records pass/fail in a ``CheckReport``.
    """
...
def _guess_base_package_name(installed_name: str) -> str | None:
    """Best-effort map from versioned on-disk name back to a ``package.json`` key.

    Used only by ``check_transitive_closure`` to walk internal amdrocm dependencies.
    Strips kpack suffixes (``-gfx*``, ``-host*``, major.minor version) before lookup.
    """
    name = installed_name
    name = re.sub(r"-dev(?=\d)", "-devel", name)
...
def _variant_to_dict(variant: VariantCheck) -> dict:
    """Serialize one ``VariantCheck`` for JSON report output."""
    return {
        "label": variant.label,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse CLI arguments for ``pkg_dependency_checker`` (default mode: ``package-file``)."""
    ...
    parser.add_argument(
        ...
        help="package-file: read local archives (pre-upload); installed: query dpkg/rpm",
    )
    parser.add_argument(
        "--pkg-type",
        required=True,
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: parse args, run checks, print/write report, return exit code.

    Exit codes: 0 pass, 1 dependency failure, 2 usage/config error.
    """
...
# Epilog
# -----
# Stage 1 delivers the pre-upload dependency gate (Milestone Step 2):
#   build_package.py output  →  pkg_dependency_checker (--mode package-file)
#   package.json expectations  ↔  Depends/Requires in built archives
#
# Post-install validation (--mode installed) and ``--transitive`` are retained
# for a later phase; pre-upload CI will call package-file mode only via
# ``pre_upload_pkg_test.py`` (planned Step 5).
#
# Configuration reuses ``build_package.create_package_config`` (INC-009) so
# kpack flags, gfx targets, and install-prefix rules match the build step.
