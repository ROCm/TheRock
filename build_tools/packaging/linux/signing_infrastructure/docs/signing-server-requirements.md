...
**Status:** Draft — v0.4
**Branch:** `users/nunnikri/signing-server-phase1`
...
### 2.6 Instance Replacement / Redeployment Mechanism — **Must-Have Before Production Cutover**

The server has no SSH or SSM access by design (§8 threat model, `signing-server-design.md` §4), which is a deliberate security posture, not an oversight — but it means there must be a *different*, already-working mechanism for deploying code or config changes before this goes to production. Without this, the only way to change anything on a live signing server would be the audited break-glass procedure, which is explicitly reserved for emergencies, not routine updates.

**This requirement blocks production cutover — it must be satisfied and tested before Phase 1 is considered done, not treated as a Phase 2 nice-to-have.**

| ID          | Requirement                                                                                                                                                                                                                                                                    |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P1-DEPLOY-1 | Code and configuration changes shall be deployed by replacing the EC2 instance, never by mutating a running instance (no in-place `git pull`, no live-editing config on the box)                                                                                               |
| P1-DEPLOY-2 | Each deployment shall bootstrap from a pinned, immutable reference (a release tag or commit SHA) — never a floating branch — so that "what code is this instance running" has an unambiguous answer at every point in time                                                     |
| P1-DEPLOY-3 | A documented, step-by-step replacement procedure shall exist (see `operations-runbook.md` §8) covering: launch new instance → bootstrap pinned to the new reference → verify `/health` before any caller traffic is routed to it → cut over → confirm → terminate old instance |
| P1-DEPLOY-4 | Callers (CI build runners, `sign-file` operators) shall be configured against a **stable address** (private DNS record or a reusable ENI) — not the signing server's raw private IP — so that an instance replacement never requires a caller-side configuration change        |
| P1-DEPLOY-5 | The replacement procedure shall support rollback by re-running itself targeting the previous known-good tag/commit — this depends on P1-DEPLOY-2 (pinned references) to be unambiguous                                                                                         |
| P1-DEPLOY-6 | This mechanism shall be exercised (a real replacement performed, not just documented) at least once before production cutover, to confirm the stable-address cutover in P1-DEPLOY-4 actually works end-to-end                                                                  |

Note: P1-DEPLOY-4 changes what `GPG_SIGNING_SERVER` in P1-CI-6 actually contains (a DNS name, not a raw IP) — update the secret's value accordingly once the stable address exists; the requirement that it be stored as a secret rather than hardcoded is unaffected.
...
| Q-6 | The instance-replacement/redeployment mechanism (§2.6, `P1-DEPLOY-*`) is documented but has not yet been exercised end-to-end — including the private-DNS/stable-ENI cutover it depends on | High — blocking for Phase 1 go-live; must be tested at least once before production cutover |
