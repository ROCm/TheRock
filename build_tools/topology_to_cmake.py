...
def generate_feature_declarations(topology: BuildTopology, f: TextIO):
    ...
    # Skip certain artifacts that don't need features
    skip_artifacts = {"sysdeps", "host-blas", "host-suite-sparse", "fftw3"}
    ...
    for artifact in artifacts_in_order:
        ...
        for dep_name in artifact.artifact_deps:
            if dep_name in skip_artifacts:
                continue
            dep_artifact = topology.artifacts.get(dep_name)
            if dep_artifact:
                requires.append(topology.get_artifact_feature_name(dep_artifact))
