...
When a pytest-xdist worker's first complex-gemm or code-object load fails,
the library caches the failure and every later call in that process fails the
same way, so no in-process rerun can clear it. A new process can, and whether
a failure survives that pass is what decides the job.

A subTest failure is recorded as failed with every phase passed and no
message, which costs nothing here: a retry needs only the nodeid.

Example usage:

    python list_failed_tests.py logs/pytest_results_single.json
"""
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="list_failed_tests.py")
    p.add_argument(
        "reports",
        nargs="+",
        type=pathlib.Path,
