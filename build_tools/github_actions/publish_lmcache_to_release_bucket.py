...
"""Upload multi-architecture LMCache wheels to a TheRock release bucket."""
...
def main(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        description="Upload multi-architecture LMCache wheels to a release bucket"
    )
    parser.add_argument(
        "--source-dir",
        required=True,
        type=Path,
        help="Local directory containing the LMCache wheel",
    )
    parser.add_argument(
        "--release-type",
        required=True,
    ...
    logger.info("LMCache wheels: %s -> %s", args.source_dir, destination.s3_uri)
    ...
    if count == 0:
        raise FileNotFoundError(f"No LMCache wheels found at {args.source_dir}")
    logger.info("Uploaded %d LMCache wheel files", count)
