...
def build_test_matrix(
    *,
    ...
    if test_level == "sanity":
        print(
            "Test level 'sanity': no self-hosted GPU test jobs will be "
            "scheduled; the build job still runs sanity_check_wheel.py on CPU"
        )
        return {"include": []}
...
def format_test_summary(
    *,
    ...
    lines.extend(
        [
            "",
            "The build job always runs `sanity_check_wheel.py` on CPU before "
            "any GPU tests.",
        ]
    )
    if test_level == "full":
        lines.append(
            "The build workflow handles the additional full-suite dispatch "
