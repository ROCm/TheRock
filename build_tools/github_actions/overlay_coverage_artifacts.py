...
def merge_tree(source: Path, dest: Path) -> None:
    """Copies the `source` tree over `dest`, replacing entries already there.

    shutil.copytree(dirs_exist_ok=True) does not work for this. It recreates
    symlinks with os.symlink(), which raises FileExistsError as soon as the
    baseline install has already put the libfoo.so -> libfoo.so.1 chain in
    place -- which it always has, since the overlay runs after a full install.
    dirs_exist_ok only forgives collisions on directories.

    Directories are merged; everything else is replaced outright, so the
    instrumented file wins over the baseline one.
    """
    dest.mkdir(parents=True, exist_ok=True)
    for entry in source.iterdir():
        target = dest / entry.name
        if entry.is_symlink():
            # exists() is False for a dangling symlink, hence both checks.
            ...
            # Unlink first: copy2 follows a symlink at the destination and
            # would write through it, overwriting the baseline file it names
            # while leaving the link itself behind.
