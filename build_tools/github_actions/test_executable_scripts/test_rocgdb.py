...
XFAILED_TESTS = {
    "Generic": [
        # ROCr runtime issue. Fixed in recent code.
        # REMOVE when the fix is included.
...
def validate_rocgdb(rocgdb_bin: Path) -> None:
    ...
    try:
        ...
        result = subprocess.run([rocgdb_bin, "--version"], check=True, env=env)
        logger.info(f"ROCgdb launcher ran successfully.")

        print_section("ROCgdb executable start", border_char="-", inline=True)
        result = subprocess.run([rocgdb_bin, "--version"], check=True)
...
def run_tests(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if optimization:
            runtest_parts += [f"CFLAGS_FOR_TARGET={optimization}"]
        if runtestflags:
