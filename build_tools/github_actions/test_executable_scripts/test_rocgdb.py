...
class TestResults:
    ...
    def cleanup_old_entries(self, compiler_label: str, tests: str) -> None:
        ...
        if not self.test_data[compiler_label]:
            print("First run. Do not need to empty the dictionary entries.")
            return

        print(f"Retry run. Cleaning dictionary entries for {tests}.")
    ...
    def update_results(
        ...
        Args:
            compiler_label (str):
                Name of the compiler/toolchain (e.g. "GCC/G++", "Clang/Clang++").
            tests (str):
        ...
        result_regex = re.compile(
            r"^(PASS|FAIL|XFAIL|ERROR|UNTESTED|UNSUPPORTED|KFAIL|UNRESOLVED): (.+)"
        )
                ...
                for line in f:
                    ...
                    if match:
                        ...
                        test_file = (
                            "unknown_test_file"
                            if status is "ERROR"
                            else self.extract_test_file(test_name)
                        )
                        ...
                        # Track the current list of failed tests. Don't include
                        # ERROR here as it doesn't usually contain the testcase
                        # filename.
        ...
        for directory in sorted(grouped.keys()):
            ...
            for filename in sorted(grouped[directory].keys()):
                ...
                if len(descriptions) == 1 and descriptions[0]:
                    print(f"        [!] {filename}: {descriptions[0]}")
    ...
    def print_comparison(self) -> None:
        ...
        for compiler_label in sorted(self.failed_tests.keys()):
            ...
            if exclusive_tests:
                ...
                print("  No exclusive failing tests.")
            print()
    ...
    def print_final_status(self, xfailed_tests: Dict[str, List[str]]) -> bool:
        """
        ...
        for compiler_label in sorted(details.keys()):
            ...
            status_symbol = "✅" if detail["passed"] else "❌"
            status_text = "PASS" if detail["passed"] else "FAIL"

            print(f"{status_symbol} {compiler_label}: {status_text}")
            print(f"   Total Failed Tests: {detail['total_failed']}")

            if detail["expected_generic"]:
                print(
                    f"❌  Ignored Failures (Generic) ({len(detail['expected_generic'])}):"
                )
            ...
            if detail["expected_compiler_specific"]:
                print(
                    f"❌  Ignored Failures ({compiler_label}) ({len(detail['expected_compiler_specific'])}):"
                )
                for test in detail["expected_compiler_specific"]:
                    print(f"      [X] {test}")

            if detail["unexpected_failures"]:
                print(f"   Unexpected Failures ({len(detail['unexpected_failures'])}):")
                for test in detail["unexpected_failures"]:
                    print(f"      [!] {test}")
            else:
                print(f"   Unexpected Failures: 0")

            if detail["unused_xfails"]:
                print(f"⚠️ Unused Ignored Failures ({len(detail['unused_xfails'])}):")
...
def print_section(
    ...
    if inline:
        ...
        print(apply_color(f"{left}{title_str}{right}"))
    else:
        # Multi-line section style.
        border = border_char * width
        title_line = f"{title:^{width}}" if center else title
        print(apply_color(border))
        print(apply_color(title_line))
        print(apply_color(border))
...
def validate_required_files(required_files: Dict[str, Path]) -> None:
    ...
    Notes:
        - Prints a check mark ✅ for files found and a cross ❌ for missing files.
...
def install_required_packages() -> None:
    ...
    """
    print("Installing dejagnu, gcc, and g++...")
    cmd = ["sudo", "apt", "install", "dejagnu", "gcc", "g++", "make", "-y"]
...
def check_executables(executables: List[str]) -> None:
    ...
    Notes:
        - Prints a ✅ when an executable is found, ❌ when missing.
        - Checks are performed using `shutil.which`.
    """
    print("Validating availability of executables...")
    paths = {exe: shutil.which(exe) for exe in executables}
    missing_executables = 0

    for exe, path in paths.items():
        if path:
            print(f"✅ {exe:15} found at: {path}")
        else:
            missing_executables += 1
            print(f"❌ {exe:15} NOT found on PATH")

    if missing_executables > 0:
        print(
            f"❌ Error: Missing {missing_executables} executables required for testing."
        )
        sys.exit(1)
...
def set_core_file_limit() -> bool:
    ...
    """
    print("Setting core file size limit to unlimited...")
    ...
    try:
        ...
        if soft == resource.RLIM_INFINITY:
            print("✅ Core file size limit set to unlimited")
            return True
        else:
            print(f"   Warning: Core file size limit is {soft}, not unlimited")
            print("   Core file tests may not execute properly")
            return False

    except ValueError as ve:
        print(f"❌ Error: Unable to set core file size limit: {ve}")
        print("   Warning: Core file tests will not be executed")
        return False

    except Exception as e:
        print(f"❌ Error: Unexpected error setting core file limit: {e}")
        print("   Warning: Core file tests will not be executed")
        return False


def cleanup_test_suite(test_suite_dir: Path, env_vars: Dict[str, str]) -> None:
    """
    ...
    """
    print("Cleaning test suite directory...")
    cmd = ["make", "clean"]
    print(f"Executing cleanup: {shlex.join(cmd)}")

    try:
        subprocess.run(cmd, cwd=str(test_suite_dir), check=True, env=env_vars)
    except subprocess.CalledProcessError as e:
        print(f"❌ Error: Failed to clean test directory: {e}")
        sys.exit(1)

    print("Removing old site.exp and site.bak...")
    ...
    try:
        ...
        print(f"❌ Error: Failed to create site.exp: {e}")
        sys.exit(1)


def configure_test_suite(test_suite_dir: Path, env_vars: Dict[str, str]) -> None:
    """
    ...
    try:
        ...
        print(f"❌ Error: Test suite configuration failed: {e}")
        sys.exit(1)

    cleanup_test_suite(test_suite_dir, env_vars)
...
def set_test_timeout(test_suite_dir: Path, timeout_value: int) -> None:
    ...
    try:
        ...
        print(f"❌ Error: {site_exp_file} not found.")
        sys.exit(1)
    except IOError as e:
        print(f"❌ Error: Failed to write to {site_exp_file}: {e}")
        sys.exit(1)
...
def expand_test_paths(test_list: List[str], testsuite_dir: Path) -> str:
    ...
    Notes:
        ...
        - If no files are found in a directory, logs a warning.
    """
    ...
    for test_path in test_list:
        ...
        if not test_path.endswith(".exp"):
            ...
            if matches:
                ...
                print(f"Expanded directory '{test_path}' to {len(matches)} test files")
            else:
                logging.warning(f"No test files found matching pattern: {pattern}")
        else:
            expanded_tests.append(test_path)
            print(f"Added test file: {test_path}")
...
def parse_arguments() -> argparse.Namespace:
  ...
  python %(prog)s --runtestflags="--target_board=hip -debug"
    ...
    if (args.testsuite_dir is None) != (args.rocgdb_bin is None):
        print(
            "❌ Error: Both --testsuite-dir and --rocgdb-bin must be provided together, or neither."
        )
        sys.exit(1)
    ...
    if not (0 < args.timeout <= 600):
        print(
            f"❌ Error: Timeout must be between 1 and 600 seconds. Got {args.timeout}."
        )
        sys.exit(1)
    ...
    if args.max_failed_retries < 0:
        print(
            f"❌ Error: Max failed retries must be non-negative. Got {args.max_failed_retries}."
        )
        sys.exit(1)
...
def run_tests(
    ...
    Args:
        ...
        cxx (str):
            C++ compiler command.
        compiler_label (str):
            Label for the compiler (e.g., "GCC/G++").
        test_results (TestResults):
            Object for storing and updating test results.
        timeout_value (int):
            Timeout (in seconds) for individual tests.
        max_failed_retries (int, optional):
            Maximum number of retries for failed tests (default: 0).
        parallel (bool, optional):
            If True, run tests in parallel using `make -j` (default: False).
        optimization (str, optional):
            Compiler optimization flags (e.g., "-O0").
        runtestflags (str, optional):
            Additional flags for `RUNTESTFLAGS`.
    ...
    """
    max_iterations = 1 + max_failed_retries
    current_tests = tests

    for iteration in range(1, max_iterations + 1):
        print(f"Running ROCgdb tests with {compiler_label} - Iteration {iteration}...")
        print(f"Number of tests to run: {len(current_tests.split())}")

        configure_test_suite(test_suite_dir, env_vars)

        # If re-running due to timeout failures, apply timeout.
        if iteration != 1 and test_results.test_data[compiler_label].get("TIMEOUT"):
            set_test_timeout(test_suite_dir, timeout_value)
        ...
        if optimization:
            runtest_parts += [
                f"CFLAGS_FOR_TARGET={optimization}",
                f"CXXFLAGS_FOR_TARGET={optimization}",
            ]
        if runtestflags:
            runtest_parts.append(runtestflags)
        ...
        subprocess.run(cmd, cwd=str(test_suite_dir), check=False, env=env_vars)
        ...
        if not failed_tests:
            print(f"✅ No failing tests for {compiler_label}. Stopping iterations.")
            break
        elif iteration < max_iterations:
            # Only rerun failed tests next time.
            current_tests = " ".join(extract_test_files(failed_tests))
            print(
                f"⚠️  {len(failed_tests)} failing test(s) found for {compiler_label}. "
                f"Proceeding to iteration {iteration + 1} with only failed tests."
            )
        else:
            print(
                f"⚠️  {len(failed_tests)} failing test(s) remain for {compiler_label} "
...
def main():
    ...
    if args.testsuite_dir is None:
        th_roc_bin_dir = validate_env_var("THEROCK_BIN_DIR")
        artifacts_dir = validate_env_var("OUTPUT_ARTIFACTS_DIR")
        rocgdb_bin = th_roc_bin_dir / "rocgdb"
    ...
    check_executables(["hipcc", "gcc", "g++", "clang", "clang++", "runtest"])

    # Resolve and expand test paths.
    try:
        tests = expand_test_paths(args.tests, rocgdb_testsuite_dir)
    except Exception as e:
        print(f"❌ Error: Failed to expand test paths: {e}")
        sys.exit(1)
    ...
    compilers = [("gcc", "g++", "GCC/G++"), ("clang", "clang++", "Clang/Clang++")]
    ...
    for cc, cxx, compiler_label in compilers:
        run_tests(
    ...
    overall_pass = test_results.print_final_status(XFAILED_TESTS)

    duration = time.perf_counter() - start_time
    print(f"Total test run duration: {duration:.4f} seconds.")
...
def print_configuration(
    ...
    """
    print("\n" + "=" * 80)
    print("ROCgdb Test Suite Configuration")
    print("=" * 80)
    print(f"  ROCgdb Binary:        {rocgdb_bin}")
    print(f"  Testsuite Directory:  {testsuite_dir}")
    print(f"  Configure Script:     {configure_script}")
    print(f"  Tests:                {' '.join(args.tests)}")
    print(f"  Parallel Execution:   {'Enabled' if args.parallel else 'Disabled'}")
    print(f"  Group Results:        {'Enabled' if args.group_results else 'Disabled'}")
    print(f"  Timeout Value:        {args.timeout} seconds")
    print(f"  Max Failed Retries:   {args.max_failed_retries}")
    print(
