...
# run exists to remove. hipThreads now sizes its scheduler from the device instead of
# multiplying a fixed vcores-per-WGP by the CU count, and we need CI on a CDNA part to
# exercise that default. HIPTHREADS_DEBUG_SIZING makes the library log the derivation
# (arch, multiprocessor count, occupancy, and the value it settled on) once per device.
...
cmd = [
    "lit",
    "-v",
    # TEMPORARY (ROCM-30565 validation, do not merge): -a shows output for passing
    # tests too. Without it the scheduler sizing line logged by HIPTHREADS_DEBUG_SIZING
    # is only visible on failure, which is precisely when we don't need it.
    "-a",
    "-j",
    "1",
