...
def resolve_amdgpu_target(amdgpu_families, amdgpu_targets=None):
    """Resolve the concrete target used for {AMDGPU_TARGETS} substitutions."""
    if amdgpu_targets is None:
        amdgpu_targets = os.getenv("AMDGPU_TARGETS")
    target = amdgpu_targets.split(",")[0].strip() if amdgpu_targets else ""
    return target or (extract_gpu_arch(amdgpu_families) or "")
...
def build_environment(rocm_path, component_name):
    ...
    if config_path.is_file():
        ...
        apply_execution_environment(
            ...
            rocm_path,
            config,
            resolve_amdgpu_target(
                os.getenv("AMDGPU_FAMILIES"), env.get("AMDGPU_TARGETS")
            ),
        )
    return env


def apply_execution_environment(env, rocm_path, config, amdgpu_target=""):
...
def run_phase(
    ...
    *,
    ...
    amdgpu_target = resolve_amdgpu_target(amdgpu_families, env.get("AMDGPU_TARGETS"))
    marker_expr = (
        marker_expression_override
        if marker_expression_override is not None
        else build_marker_expression(category_config, gpu_arch, amdgpu_target)
    )

    # Extra pytest CLI options for this category, with runtime substitutions.
    pytest_args = (
    ...
    category_timeout = (
        exec_settings.get("category_timeouts", {}).get(test_type) if test_type else None
    )
    ...
    junit_xml = str(Path(junit_dir) / f"{component_name}.xml") if junit_dir else None

    apply_execution_environment(env, rocm_path, config, amdgpu_target)
