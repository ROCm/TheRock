...
# TODO: Dapper: search filesystem for tests_to_run and load gtest_filter if found


def set_gtest_filter(filter: str = None):
    # if filter is given, it's already in --gtest_filter form, so just store it
    if filter:
        test_filter = filter
    else:
        # Add default positive filters
        # Fusion #
