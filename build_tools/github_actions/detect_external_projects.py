...
def get_changed_subtrees(modified_paths: List[str], subtree_to_project_map: dict) -> Set[str]:
...
def detect_projects_from_changes(
    ...
    # For PRs and pushes, detect based on changed files
        ...
        if not check_for_non_skippable_path(modified_paths):
            print("Only skippable paths modified (docs, markdown, etc.) - skipping all builds")
            return []
        
        # Find which project subtrees were modified
        subtrees_to_build = get_changed_subtrees(
            modified_paths,
        ...
        if not subtrees_to_build:
            ...
            print(f"Note: Modified paths don't match any known projects in subtree_to_project_map")
    ...
    print(f"Generated {len(project_configs)} project configuration(s) for platform {platform}")
...
def main():
    ...
    try:
        ...
        print(f"ERROR: Failed to import external_repo_project_maps: {e}", file=sys.stderr)
        sys.exit(1)
