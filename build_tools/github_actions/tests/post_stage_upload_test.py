...
class TestMainCli(unittest.TestCase):
    ...
    def test_platform_override_controls_upload_path(self):
        """Verify --platform overrides the host platform in upload paths."""
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as staging:
            build_dir = Path(tmp)
            staging_dir = Path(staging)
            log_dir = build_dir / "logs"
    ...
    @mock.patch("post_stage_upload.platform.system", return_value="Windows")
    def test_default_platform_uses_current_system(self, _mock_system):
        """Verify omitted --platform preserves current-system behavior."""
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as staging:
            build_dir = Path(tmp)
            staging_dir = Path(staging)
            log_dir = build_dir / "logs"
