...
class PreparePyTorchManifestsTest(unittest.TestCase):
    ...
    def test_parses_s3_manifest_directory_locations(self) -> None:
        self.assertEqual(
            m.storage_location_from_url("s3://test-bucket/run-1/manifests/pytorch/"),
            m.StorageLocation(
                bucket="test-bucket",
                relative_path="run-1/manifests/pytorch",
            ),
        )
        self.assertEqual(
            m.storage_location_from_url(
                "https://test-bucket.s3.amazonaws.com/run-1/manifests/pytorch/index.html"
            ),
            m.StorageLocation(
                bucket="test-bucket",
                relative_path="run-1/manifests/pytorch",
    ...
    def test_main_generates_single_manifest_and_writes_url(self) -> None:
        ...
        summary = gha_append_step_summary.call_args.args[0]
        self.assertIn("## PyTorch Manifests", summary)
        self.assertIn("index.html", summary)
        self.assertIn("therock-manifest_torch_linux_release-2.12.json", summary)

    def test_main_consumes_manifest_directory_url_for_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as staging:
            staging_dir = Path(staging)
            manifest_dir = staging_dir / "run-1" / "manifests" / "pytorch"
            ...
            with mock.patch.object(
                ...
                m.main(
                    [
                        ...
                        "matrix",
                        "--manifest-dir-url",
                        "https://test.s3.amazonaws.com/run-1/manifests/pytorch/",
                        "--pytorch-git-refs",
                ...
                "include": [
                    {
                        ...
                        "manifest_url": (
                            ...
                            "run-1/manifests/pytorch/release.json"
                        ),
                    },
                    {
                        "python_version": "3.12",
                        ...
                        "manifest_url": (
                            "https://test.s3.amazonaws.com/"
                            "run-1/manifests/pytorch/nightly.json"
                        ),
                    },
                ]
            },
        ...
        self.assertEqual(
            output["manifest_dir_s3_uri"],
            "s3://test/run-1/manifests/pytorch",
        )
        summary = gha_append_step_summary.call_args.args[0]
        self.assertNotIn("### Inputs", summary)
        self.assertIn("* ROCm version: `7.13.0`", summary)
        self.assertIn("### Manifest Files", summary)
        self.assertIn("### Build Matrix", summary)
        self.assertIn("| PyTorch ref | Python version |", summary)
        self.assertIn("release.json", summary)

    def test_main_generates_release_matrix_with_list_inputs(self) -> None:
        def fake_generate_manifest(**kwargs):
            return {
                "pytorch": {
                    "repo": "https://github.com/ROCm/pytorch",
        ...
        self.assertIn("* ROCm version: `7.13.0`", summary)
        self.assertIn("| PyTorch ref | Python version |", summary)
        self.assertIn("| `release/2.8` | `3.13` |", summary)
        self.assertIn("| `nightly` | `3.14` |", summary)
        self.assertIn("therock-manifest_torch_linux_release-2.8.json", summary)
        self.assertEqual(
            [call.kwargs["projects"] for call in generate_manifest.call_args_list],
            [
                ["pytorch", "pytorch_audio"],
