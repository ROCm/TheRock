...
import test_profiler_storage_host_asan
from _therock_utils.build_topology import get_topology


class ProfilerStorageHostAsanTest(unittest.TestCase):
    ...
    def test_component_inventory_is_explicit(self):
        self.assertEqual(
            set(test_profiler_storage_host_asan.GTEST_COMPONENTS),
            {"rocprofiler-compute", "rocprofiler-sdk"},
        )
        self.assertEqual(
            set(test_profiler_storage_host_asan.CTEST_COMPONENTS), {"hipfile"}
        )
        self.assertEqual(
            [
                entry["inventory_count"]
                for entry in test_profiler_storage_host_asan.GTEST_COMPONENTS[
                    "rocprofiler-compute"
                ]
            ],
            [79, 59],
        ...
        self.assertEqual(
            test_profiler_storage_host_asan.GTEST_COMPONENTS["rocprofiler-sdk"][1][
                "skipped_names"
            ],
            ("codeobj_library.dwarf_matches_llvm_symbolizer",),
        )
        self.assertEqual(
            test_profiler_storage_host_asan.CTEST_COMPONENTS["hipfile"][
                "inventory_count"
            ],
            668,
        )
...
"""
        self.assertEqual(
            test_profiler_storage_host_asan._parse_gtest_names(output),
            ["Suite.one", "Typed/Parameterized.two/0"],
        )
        self.assertEqual(
            test_profiler_storage_host_asan._normalize_names(
                ["Suite.two", "Suite.one"]
            ),
            "Suite.one\nSuite.two\n",
        )
    ...
    def test_test_environment_forbids_preload_and_enables_lsan(self):
        with patch.dict(
            ...
            env = test_profiler_storage_host_asan._test_environment(
        ...
        with patch.dict(
            ...
            trace_env = test_profiler_storage_host_asan._test_environment(
    ...
    def test_direct_asan_check_uses_shared_instrumentation_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            executable = Path(tmp) / "test"
            executable.touch()
            with patch.object(
                test_profiler_storage_host_asan,
                "require_direct_clang_asan",
            ) as require_direct:
                test_profiler_storage_host_asan._require_direct_asan(executable, {})
    ...
    def test_inventory_digest_mismatch_fails_before_execution(self):
        with self.assertRaisesRegex(RuntimeError, "inventory changed before execution"):
            test_profiler_storage_host_asan._check_inventory(
                names=["Suite.test"],
    ...
    def test_main_reports_captured_subprocess_diagnostics(self):
        ...
        with (
            ...
            patch.object(test_profiler_storage_host_asan, "_require_no_gpu_nodes"),
            patch.object(
                test_profiler_storage_host_asan,
            ...
            self.assertEqual(test_profiler_storage_host_asan.main(), 1)
...
"""
        self.assertEqual(
            test_profiler_storage_host_asan._SKIPPED_NAME_RE.findall(output),
            ["Suite.expected_skip"],
        )
    ...
    def test_main_routes_only_phase5_components(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(test_profiler_storage_host_asan, "_require_no_gpu_nodes"),
                patch.object(
                    test_profiler_storage_host_asan, "_run_ctest_component"
                ) as run,
            ):
                self.assertEqual(test_profiler_storage_host_asan.main(), 0)
