...
class FindOpenBumpPrTest(unittest.TestCase):
    def _make_pr(self, number: int, labels: list[str]) -> dict:
        return {
            "number": number,
            "labels": [{"name": l} for l in labels],
            "head": {"ref": f"bump-branch-{number}"},
        }

    def test_returns_pr_with_matching_label(self):
        prs = [
            self._make_pr(1, ["ci:run-all-archs"]),
            self._make_pr(2, ["autobump:rocgdb", "ci:run-all-archs"]),
            self._make_pr(3, ["other-label"]),
        ]
        with patch("bump_automation.gh_api", return_value=prs):
            result = find_open_bump_pr("token", "autobump:rocgdb")
    ...
    def test_returns_none_when_no_open_prs(self):
        with patch("bump_automation.gh_api", return_value=[]):
            result = find_open_bump_pr("token", "autobump:rocgdb")
        self.assertIsNone(result)

    def test_returns_first_match_when_multiple(self):
        prs = [
            self._make_pr(1, ["autobump:rocgdb"]),
            self._make_pr(2, ["autobump:rocgdb"]),
        ]
        with patch("bump_automation.gh_api", return_value=prs):
            result = find_open_bump_pr("token", "autobump:rocgdb")
...
class CloseStatePrsTest(unittest.TestCase):
