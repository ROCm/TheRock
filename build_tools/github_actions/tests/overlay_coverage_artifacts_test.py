...
class OverlayRelpathsTest(TempDirTestBase):
    ...
    @unittest.skipIf(is_windows(), "symlinks require elevation on Windows")
    def test_replaces_a_symlink_the_baseline_already_installed(self):
        # The overlay always runs after a full baseline install, so the whole
        # symlink chain is already on disk. shutil.copytree used to fail here
        # with "File exists: 'libhiprand.so.1' -> '.../libhiprand.so'" --
        # dirs_exist_ok forgives directory collisions, not symlink ones.
        stage = "staging/rand_lib_gfx942/math-libs/hipRAND/stage/lib"
        self.write(f"{stage}/libhiprand.so.1", "instrumented")
        (self.root / stage / "libhiprand.so").symlink_to("libhiprand.so.1")
        self.write("install/lib/libhiprand.so.1", "baseline")
        (self.root / "install/lib/libhiprand.so").symlink_to("libhiprand.so.1")
        ...
        link = self.root / "install/lib/libhiprand.so"
        self.assertTrue(link.is_symlink())
        self.assertEqual(link.read_text(), "instrumented")

    @unittest.skipIf(is_windows(), "symlinks require elevation on Windows")
    def test_replaces_a_baseline_regular_file_with_a_symlink(self):
        # Some baselines ship the resolved file where the instrumented artifact
        # ships a link. Leaving the file in place would keep the loader on
