...
import test_rocr_rocshmem_host_asan


class RocrRocshmemHostAsanTest(unittest.TestCase):
    ...
    def test_inventory_is_explicit(self):
        self.assertEqual(
            set(test_rocr_rocshmem_host_asan.COMPONENTS),
            {"rocshmem", "rocrtst"},
        )
        self.assertEqual(
            [
                entry["count"]
                for entry in test_rocr_rocshmem_host_asan.COMPONENTS["rocshmem"]
            ],
            [79],
        )
        self.assertEqual(
            [
                entry["count"]
                for entry in test_rocr_rocshmem_host_asan.COMPONENTS["rocrtst"]
            ],
            [9, 5, 13],
        )
...
"""
        self.assertEqual(
            test_rocr_rocshmem_host_asan._parse_gtest_names(output),
    ...
    def test_environment_removes_preload_and_enables_lsan(self):
        with patch.dict(
            ...
            env = test_rocr_rocshmem_host_asan._test_environment()
    ...
    def test_direct_asan_uses_shared_instrumentation_gate(self):
        ...
        with patch.object(
            test_rocr_rocshmem_host_asan,
            "require_direct_clang_asan",
        ) as require_direct:
            test_rocr_rocshmem_host_asan._require_direct_asan(executable, {})
    ...
    def test_inventory_drift_fails_before_test_execution(self):
        ...
        with (
            patch.object(test_rocr_rocshmem_host_asan, "_require_direct_asan"),
            patch.object(
                test_rocr_rocshmem_host_asan.subprocess,
                "run",
                return_value=listed,
            ) as run,
        ):
            with self.assertRaisesRegex(RuntimeError, "inventory changed"):
                test_rocr_rocshmem_host_asan._run_component(prefix, "rocshmem", {})
            self.assertEqual(run.call_count, 1)

    def test_child_failure_is_propagated(self):
        test = test_rocr_rocshmem_host_asan.COMPONENTS["rocshmem"][0]
        names = [f"Suite.case{i}" for i in range(test["count"])]
        prefix = Path("/fake/prefix")
        original_digest = test["sha256"]
        test["sha256"] = test_rocr_rocshmem_host_asan._inventory_digest(names)
        listed = Mock(
    ...
    def test_visible_gpu_nodes_fail_closed(self):
        with patch.object(Path, "exists", return_value=True):
            with self.assertRaisesRegex(RuntimeError, "unexpectedly exposes GPU nodes"):
                test_rocr_rocshmem_host_asan._require_no_gpu_nodes()
