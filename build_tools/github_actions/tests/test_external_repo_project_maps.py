...
class TestRocmLibrariesPaths(unittest.TestCase):
    ...
    @unittest.skipIf(
        not ROCM_LIBRARIES_ROOT.exists(),
        "rocm-libraries submodule not initialized"
    )
...
class TestRocmSystemsPaths(unittest.TestCase):
    ...
    @unittest.skipIf(
        not ROCM_SYSTEMS_ROOT.exists(),
        "rocm-systems submodule not initialized"
    )
    def test_subtree_paths_exist(self):
...
class TestCollectProjectsToRun(unittest.TestCase):
    ...
    def test_platform_specific_flags(self):
        ...
        # miopen has linux-specific composable_kernel flags
        ...
        # Linux should have composable_kernel flags
        linux_opts = projects_linux[0]["cmake_options"]
        self.assertIn("-DTHEROCK_ENABLE_COMPOSABLE_KERNEL=ON", linux_opts)
        
        # Windows should not have composable_kernel flags
        windows_opts = projects_windows[0]["cmake_options"]
        self.assertNotIn("-DTHEROCK_ENABLE_COMPOSABLE_KERNEL=ON", windows_opts)
