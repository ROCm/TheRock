...
class ROCblasBenchmark(BenchmarkBase):
    ...
    def _run_gemm_benchmarks(self, config_data: Dict, iterations: int, cold_iterations: int) -> None:
        """Run GEMM benchmarks."""
        try:
                    ...
                    for size in gemm_sizes:
                        ...
                        for trans in gemm_trans:
                            ...
                            cmd = [
                                ...
                                "--transposeB", trans,
                                "-i", str(iterations),
                                "-j", str(cold_iterations)
                            ]
                            
                            # Add precision-specific parameters
                            if precision == 'h':
                                cmd.extend(["--alpha", "1", "--beta", "0", "--transposeA", "N"])
    ...
    def _run_gemv_benchmarks(self, config_data: Dict, iterations: int, cold_iterations: int) -> None:
        """Run GEMV benchmarks."""
        try:
            ...
            with open(log_file, "w+") as f:
                ...
                if not (len(gemv_m) == len(gemv_n) == len(gemv_lda)):
                    log.warning(f"GEMV config length mismatch: m={len(gemv_m)}, n={len(gemv_n)}, lda={len(gemv_lda)}")
    ...
    def _run_ger_benchmarks(self, config_data: Dict, iterations: int, cold_iterations: int) -> None:
        """Run GER benchmarks."""
        try:
            ...
            with open(log_file, "w+") as f:
                ...
                if not (len(ger_m) == len(ger_n) == len(ger_lda)):
                    log.warning(f"GER config length mismatch: m={len(ger_m)}, n={len(ger_n)}, lda={len(ger_lda)}")
    ...
    def _run_dot_benchmarks(self, config_data: Dict, iterations: int, cold_iterations: int) -> None:
        """Run DOT benchmarks."""
        try:
    ...
    def _run_gemm_hpa_hgemm_benchmarks(self, config_data: Dict, iterations: int, cold_iterations: int) -> None:
        ...
        try:
            ...
            with open(log_file, "w+") as f:
                ...
                gemm_hpa_hgemm_compute_type = gemm_hpa_hgemm_config.get("compute_type", "s")
                ...
                for size in gemm_hpa_hgemm_sizes:
                    ...
                    for trans in gemm_hpa_hgemm_trans:
                        ...
                        cmd = [
                            ...
                            "--ldc", str(ldc),
                            "--ldd", str(ldd),
                            "--compute_type", gemm_hpa_hgemm_compute_type,
                            "--transposeB", trans,
                            "-i", str(iterations),
                            "-j", str(cold_iterations)
                        ]
            ...
            try:
                ...
                while i < len(lines):
                    ...
                    if 'rocblas-bench' in line and '-r' in line:
                        ...
                        try:
                            idx = parts.index('-r')
                            current_precision = parts[idx + 1] if idx + 1 < len(parts) else None
                    ...
                    if "rocblas-Gflops" in line:
                        ...
                        if len(values) != len(header):
                            log.warning(f"Column mismatch: expected {len(header)}, got {len(values)}")
                            i += 1
                            continue
                        ...
                        if current_precision and not any(k in params for k in ['a_type', 'precision']):
                            params['precision'] = current_precision
                        
                        function_type = self._determine_function_type(params)
                        subtest_name = self._build_subtest_name_from_params(function_type, params)
                        
                        try:
                            gflops = float(params.get('rocblas-Gflops', '0'))
                            status = "PASS" if gflops > 0 else "FAIL"
                            
                            row_data = [self.benchmark_name, subtest_name, num_gpus, status, gflops, 'rocblas-Gflops', 'H']
                            suite_table.add_row(row_data)
                            
                            test_results.append(self.create_test_result(
                                self.benchmark_name, subtest_name, status, gflops, "rocblas-Gflops", "H",
                                ngpu=num_gpus
                            ))
    ...
    def _build_subtest_name_from_params(self, function_type: str, params: Dict[str, str]) -> str:
        ...
        def get_precision() -> str:
            ...
            precision = get_param('a_type') or get_param('precision') or get_param('compute_type')
