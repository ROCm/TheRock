    ...
    def parse_results(self) -> Tuple[List[Dict[str, Any]], PrettyTable]:
        ...
        try:
            ...
            for line in data[header_index + 1:]:
                ...
                table.add_row([self.benchmark_name, subtest_name, batch_count, num_gpus, status, score, 'Gflops', 'H'])
                test_results.append(self.create_test_result(
                    self.benchmark_name, subtest_name, status, score, "Gflops", "H",
                    batch_size=batch_count, ngpu=num_gpus
                ))
