        ...
        try:
            with open(self.log_file, 'r') as log_fp:
                ...
                for line in log_fp:
                    ...
                    table.add_row([self.benchmark_name, time_testname, batch_size, num_gpus, status, gpu_time, 'ms', 'L'])
                    test_results.append(self.create_test_result(
                        self.benchmark_name, time_testname, status, gpu_time, "ms", "L",
                        batch_size=batch_size, ngpu=num_gpus
                    ))
                    
                    # Add GFLOPS result
                    gflops_testname = f"rider_{subtest_id}_gflops"
                    table.add_row([self.benchmark_name, gflops_testname, batch_size, num_gpus, status, gflops, 'GFLOPS', 'H'])
                    test_results.append(self.create_test_result(
                        self.benchmark_name, gflops_testname, status, gflops, "GFLOPS", "H",
                        batch_size=batch_size, ngpu=num_gpus
                    ))
