...
class MPIHelper:
    ...
    def _build_mpi(self) -> bool:
        ...
        try:
            ...
            subprocess.run(["wget", "-q", "-O", str(tar_file), mpi_url], 
                         check=True, timeout=300)
            subprocess.run(["tar", "-xzf", str(tar_file), "-C", str(build_dir)], 
            ...
            subprocess.run(["./configure", f"--prefix={self.install_dir}", "--enable-mpi-cxx", 
                          "--with-rocm", "--disable-man-pages"], 
                         check=True, timeout=600, **common_opts)
