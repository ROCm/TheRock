...
def upload_artifacts(
    ...
    count = backend.upload_directory(
        artifacts_dir, run_root.root(), include=["*.tar.xz*"]
    )
    ...
    if index_path.is_file():
        backend.upload_file(index_path, run_root.artifact_index(artifact_group))
...
def upload_logs(
    ...
    # Upload all log files. Content-type is inferred per file by the backend
    # (e.g., .log → text/plain, .html → text/html, .tar.gz → application/gzip).
    log("Uploading logs")
    backend.upload_directory(log_dir, run_root.log_dir(artifact_group))
    ...
    if resource_prof_dir.is_dir():
        for filename in ["comp-summary.html", "comp-summary.md"]:
            ...
            if file_path.is_file():
                backend.upload_file(
                    file_path, run_root.log_file(artifact_group, filename)
                )
...
def upload_manifest(
    ...
    backend.upload_file(manifest_path, run_root.manifest(artifact_group))
...
def run(args):
    ...
    if not args.job_status or args.job_status == "success":
        ...
        log("----------------------")
        upload_artifacts(args.artifact_group, args.build_dir, run_root, backend)

    log("Upload log")
    log("----------")
    upload_logs(args.artifact_group, args.build_dir, run_root, backend)

    log("Upload manifest")
    log("----------------")
    upload_manifest(args.artifact_group, args.build_dir, run_root, backend)
