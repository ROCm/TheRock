    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                log(
                    f"[WARN] Retry {attempt + 1}/{max_retries} for aws command: {e}"
                )
...
def run_aws_cp(
    ...
    try:
        ...
        run_aws_command(cmd, cwd=Path.cwd(), max_retries=max_retries)
    except subprocess.CalledProcessError as e:
...
def upload_artifacts(
    artifact_group: str, build_dir: Path, bucket_uri: str, max_retries: int
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Maximum number of retries for AWS uploads (default: 3)",
    )
