...
class RockProjectRepo:
    ...
    def __exec_subprocess_cmd(self, exec_cmd):
        ...
        if exec_cmd is not None:
            ...
            result = subprocess.run(
                exec_cmd, shell=True, capture_output=False, text=True
            )
    ...
    def do_checkout(
        ...
        self.exec(
            ["git", "fetch"] + fetch_args + ["origin", self.project_version_hashtag],
            cwd=self.project_src_dir,
        )
        ...
        if install_cmd is not None:
            if "ROCK_CONFIG_CMD__FIND_AND_INSTALL_LATEST_PYTHON_WHEEL" in install_cmd:
                ...
                if len(install_cmd_arr) == 2:
                    ...
                    res = subprocess.run(
                        inst_cmd, shell=True, capture_output=False, text=True
                    )
